...
try:
    ...
    print(
        "Uyarı: Numba kütüphanesi bulunamadı. Benzerlik hesaplamaları için Numba hızlandırması kullanılamayacak."
    )
...
try:
    ...
    print(
        "Uyarı: PyTorch kütüphanesi bulunamadı. CUDA benzerlik hesaplaması yapılamayacak."
    )
...
if PYTORCH_AVAILABLE:
    ...
    def cosine_similarity_cuda(
        vec1_tensor: torch.Tensor, vec2_tensor: torch.Tensor
    ) -> float:
        ...
        return F.cosine_similarity(
            vec1_tensor.unsqueeze(0), vec2_tensor.unsqueeze(0)
        ).item()

    def euclidean_similarity_cuda(
        vec1_tensor: torch.Tensor, vec2_tensor: torch.Tensor
    ) -> float:
    ...
    def manhattan_similarity_cuda(
        vec1_tensor: torch.Tensor, vec2_tensor: torch.Tensor
    ) -> float:
        """İki PyTorch tensörü arasında Manhattan benzerliğini CUDA üzerinde hesaplar."""
        distance = torch.dist(
            vec1_tensor, vec2_tensor, p=1
        ).item()  # p=1 Manhattan mesafesidir
        if distance < 1e-10:
            return 1.0
        return 1.0 / (1.0 + distance)
    ...
    cosine_similarity_cuda = cosine_similarity_numba
    euclidean_similarity_cuda = euclidean_similarity_numba
    manhattan_similarity_cuda = manhattan_similarity_numba
    SIMILARITY_FUNCTIONS_CUDA = {
        "cosine": cosine_similarity_cuda,
        "euclidean": euclidean_similarity_cuda,
        "manhattan": manhattan_similarity_cuda,
...
def calculate_similarity(
    ...
    try:
        ...
        print(
            "--- Uyarı: Flask context dışında çalışılıyor, CUDA kullanılamayacak. ---"
        )
    ...
    if use_cuda and actual_use_cuda and pytorch_available_runtime:
        ...
        if cuda_func:
            try:
                ...
                print(
                    f"--- UYARI: '{selected_algorithm}' için CUDA implementasyonu yok. Numba'ya fallback yapılıyor. ---"
                )  # Uyarı kalsın
            except Exception as cuda_err:
                print(
                    f"--- HATA: CUDA hesaplaması sırasında hata: {cuda_err}. Numba'ya fallback yapılıyor. ---"
                )  # Hata mesajı kalsın
                # CUDA hatasında Numba'ya düşebiliriz.
        else:
            print(
                f"--- UYARI: '{selected_algorithm}' için CUDA fonksiyonu bulunamadı. Numba'ya fallback yapılıyor. ---"
            )  # Uyarı kalsın
    ...
    if numba_func and NUMBA_AVAILABLE:
        ...
        numba_func
    ):  # Numba kurulu değilse NumPy'ı manuel kullanalım (calculate_similarity içindeki eski kod)
        ...
        raise ValueError(
            f"Desteklenmeyen benzerlik algoritması: '{algorithm}'. Seçenekler: {list(SIMILARITY_FUNCTIONS_NUMBA.keys())}"
        )
...
if __name__ == "__main__":
    ...
    print(
        f"Benzerlik (A, B): {calculate_similarity(vec_a, vec_b, algorithm='euclidean')}"
    )
    print(
        f"Benzerlik (A, A): {calculate_similarity(vec_a, vec_a, algorithm='euclidean')}"
    )
    print(
        f"Benzerlik (A, C): {calculate_similarity(vec_a, vec_c, algorithm='euclidean')}"
    )

    print("--- Manhattan Benzerliği --- ")
    print(
        f"Benzerlik (A, B): {calculate_similarity(vec_a, vec_b, algorithm='manhattan')}"
    )
    print(
        f"Benzerlik (A, A): {calculate_similarity(vec_a, vec_a, algorithm='manhattan')}"
    )
    print(
        f"Benzerlik (A, C): {calculate_similarity(vec_a, vec_c, algorithm='manhattan')}"
    )

    print("--- Sıfır Vektör Testleri --- ")
    print(
        f"Kosinüs (A, Zero): {calculate_similarity(vec_a, vec_zero, algorithm='cosine')}"
    )
    print(
        f"Öklid (A, Zero): {calculate_similarity(vec_a, vec_zero, algorithm='euclidean')}"
    )
    print(
        f"Manhattan (A, Zero): {calculate_similarity(vec_a, vec_zero, algorithm='manhattan')}"
    )

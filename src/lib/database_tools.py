...
MILVUS_HOST = "localhost"  # VEYA ENV'den oku
MILVUS_PORT = "19530"    # VEYA ENV'den oku
MILVUS_ALIAS_APP = "eow_milvus_app" # Uygulama için farklı bir alias
...
def get_default_db_config():
    # Bu fonksiyonun gerçek implementasyonu ortamınıza bağlı olacaktır.
    # Şimdilik sabit değerler kullanıyoruz, idealde config dosyasından/env'den okunmalı.
    return {
        'host': 'localhost',
        'port': '5432',
        'user': 'postgres',
        'password': 'your_password',
        'database': 'EyeOfWebMilvus' # Varsayılan olarak yeni DB
    }
...
def connect_to_milvus(alias=MILVUS_ALIAS_APP):
    try:
        if not connections.has_connection(alias):
            p_info(f"Connecting to Milvus at {MILVUS_HOST}:{MILVUS_PORT} under alias '{alias}'...")
            connections.connect(alias=alias, host=MILVUS_HOST, port=MILVUS_PORT, user=MILVUS_USER, password=MILVUS_PASSWORD)
...
def get_milvus_collection(collection_name: str, alias: str = MILVUS_ALIAS_APP) -> typing.Optional[Collection]:
    """
    ...
    try:
        if not utility.has_collection(collection_name, using=alias):
            p_error(f"Milvus collection '{collection_name}' does not exist under alias '{alias}'.")
            return None
        ...
        p_log(f"Collection '{collection_name}': Initial raw_load_state='{raw_current_load_state}', "
        ...
        if not is_currently_loaded:
            p_warn(f"Collection '{collection_name}' is not fully loaded (State: '{current_load_state_str}'). Attempting to load...")
            try:
                fields_to_load = []
                ...
                if collection_name in [
                    ...
                    p_info(f"Attempting to load collection '{collection_name}' for specific vector fields: {fields_to_load}")
                    collection.load(field_names=fields_to_load, _async=False) # _async=False senkron yükleme için
                else:
                    # Bilinmeyen veya farklı yapıdaki koleksiyonlar için genel yükleme
                    p_info(f"Attempting a generic load for collection '{collection_name}'.")
                    collection.load(_async=False) # _async=False senkron yükleme için
                
                # Yükleme sonrası durumu tekrar kontrol et
                utility.wait_for_loading_complete(collection_name, using=alias) # Yüklemenin bitmesini bekle
                raw_final_load_state = utility.load_state(collection_name, using=alias)
                final_load_state_str = str(raw_final_load_state).strip()

                p_log(f"Collection '{collection_name}': Post-load attempt raw_load_state='{raw_final_load_state}', "
                ...
                is_finally_loaded = (final_load_state_str == "Loaded") # String ile kontrol
                ...
                if is_finally_loaded:
                    ...
                    p_error(f"Failed to load Milvus collection '{collection_name}'. "
                    ...
                    if "Loaded" in final_load_state_str and not is_finally_loaded:
                         p_warn(f"WORKAROUND: Collection '{collection_name}' reported as '{final_load_state_str}' "
                ...
                p_error(f"MilvusException during load attempt for '{collection_name}': {load_err}")
                traceback.print_exc()
                return None
            except Exception as e:
                p_error(f"Unexpected error during load attempt for '{collection_name}': {e}")
                traceback.print_exc()
                return None
        else:
            p_log(f"Collection '{collection_name}' is already loaded (State: '{current_load_state_str}').")

        return collection

    except CollectionNotExistException: # pymilvus.exceptions altından import edildiğinden emin olun
        p_error(f"Milvus collection '{collection_name}' does not exist (caught by CollectionNotExistException).")
        return None
    except MilvusException as e:
...
class DatabaseTools:
    ...
    def _initialize_milvus_connections(self):
        ...
        try:
            ...
            if not connect_to_milvus(MILVUS_ALIAS_APP):
                p_error("Ana Milvus bağlantısı kurulamadı! Koleksiyonlar yüklenemeyecek.")
                return
            ...
            for collection_name in common_collections:
                try:
                    ...
                    if collection:
                        ...
                        p_info(f"Milvus koleksiyonu önbelleğe alındı: {collection_name}")
                    else:
                        p_warn(f"Milvus koleksiyonu yüklenemedi: {collection_name}")
                except Exception as e:
                    p_error(f"Milvus koleksiyonu {collection_name} önbelleğe alınırken hata: {e}")
            
            p_info(f"Milvus başlatma tamamlandı. {len(self._milvus_collections_cache)} koleksiyon önbelleğe alındı.")
        except Exception as e:
    ...
    def insert_is_crawled(self,target_url:str) -> bool:
        ...
        try:
            ...
            STATIC_SQL_COMMAND = """SELECT "ID" FROM "BaseDomainID" WHERE "Domain" = %s"""
            ...
            if _path != None:
                STATIC_SQL_COMMAND = """SELECT "ID" FROM "UrlPathID" WHERE "Path" = %s"""
            ...
            STATIC_DATA_TUPLE = (_protocol, _baseDomainReturnID, _pathReturnID, _pathReturnID, _etcReturnID, _etcReturnID)
            ...
            STATIC_DATA_TUPLE = (_protocol, _baseDomainReturnID, _pathReturnID, _etcReturnID)
    ...
    def releaseConnection(self, conn: psycopg2.extensions.connection, cursor: psycopg2.extensions.cursor):
        if cursor:
            cursor.close()
        if conn:
            conn.close()
    ...
    def insertPageBased(self,
        ...
        try:
            ...
            STATIC_SQL_COMMAND = """SELECT "ID" FROM "BaseDomainID" WHERE "BaseDomain" = %s"""
            ...
            if categortyNmae != None:
                ...
                STATIC_SQL_COMMAND = """SELECT "ID" FROM "WebSiteCategoryID" WHERE "CategoryName" = %s"""
            ...
            if phoneNumber_list != None:
                ...
                for phone_num in phoneNumber_list:
                    ...
                    STATIC_SQL_COMMAND = """SELECT "ID" FROM "PhoneNumberID" WHERE "PhoneNumber" = %s"""
            ...
            """
            cursor.execute(STATIC_SQL_COMMAND, (protocol, baseDomain_return_id, urlPath_return_id, urlPath_return_id, urlPathEtc_return_id, urlPathEtc_return_id, categortyNmae_return_id, categortyNmae_return_id))
            result_is = cursor.fetchall()
            ...
            if len(result_is) < 1:
                ...
                cursor.execute(STATIC_SQL_COMMAND, (
                    ...
                    phoneNumber_return_id_LIST if phoneNumber_return_id_LIST else None,
                    emailAddress_return_id_LIST if emailAddress_return_id_LIST else None
                ))
                result_is = cursor.fetchall()[0]["ID"]
                conn.commit()
    ...
    def insertImageBased(self,
        ...
        milvus_face_collection = get_milvus_collection(EYE_OF_WEB_FACE_DATA_MILVUS_COLLECTION_NAME)
        ...
        try:
            ...
            if protocol is None or baseDomain is None:
                return (False, "Protocol and BaseDomain are required for the source page.")
            ...
            if save_image is True:
                if imageBinary is None:
                    self.releaseConnection(conn, cursor) # Ensure release before early return
                    return (False, "ImageBinary is required when save_image is True")
                
                compressed_image = compress_image(imageBinary) 
                
                STATIC_SQL_COMMAND = """SELECT "ID" FROM "ImageID" WHERE "BinaryImage" = %s"""
            ...
            if category is not None:
                ...
                STATIC_SQL_COMMAND = """SELECT "ID" FROM "WebSiteCategoryID" WHERE "Category" = %s"""
            ...
            if imageHash is not None:
                STATIC_SQL_COMMAND = """SELECT "ID" FROM "ImageHashID" WHERE "ImageHash" = %s"""
                ...
                if len(result_is) < 1:
                    ...
                    conn.commit()
                    p_log(f"New image hash {imageHash} (ID: {imageHash_return_id}) inserted.")
                    skip_milvus_insert = False # New hash, Milvus insert needed if faces provided
                else:
                    imageHash_return_id = result_is[0]["ID"]
                    p_log(f"Image hash {imageHash} (ID: {imageHash_return_id}) already exists. Checking for linked faces in ImageBasedMain.")
                    cursor.execute("""
                    ...
                    if current_pg_face_ids_for_hash:
                        p_info(f"Found existing PostgreSQL FaceIDs {list(current_pg_face_ids_for_hash)} for hash ID {imageHash_return_id}. These will be used. Milvus insert will be skipped.")
                        face_sql_ids_for_imagebasedmain = list(current_pg_face_ids_for_hash) # Populate with existing SQL FaceIDs
                        skip_milvus_insert = True
                    else:
                        p_log(f"Image hash ID {imageHash_return_id} exists, but no ImageBasedMain records with faces found for it (or existing records have no faces). Milvus insert will proceed if faces are provided.")
                        skip_milvus_insert = False # Ensure Milvus insert happens if faces are given
            else: 
                p_error("imageHash is None, which is required for ImageBasedMain.HashID.")
                self.releaseConnection(conn, cursor)
            ...
            if not skip_milvus_insert and faces:
                p_log("Preparing face data for new Milvus insertion as skip_milvus_insert is False and faces are provided.")
                ...
                p_log(f"Milvus insert skipped. Using existing FaceIDs: {face_sql_ids_for_imagebasedmain} for hash ID {imageHash_return_id}.")
            elif not faces: # This case means skip_milvus_insert is False, but no faces were provided.
                p_log("No faces provided, Milvus insertion skipped. `face_sql_ids_for_imagebasedmain` will be empty unless hash reuse populated it (which it wouldn't if skip_milvus_insert is false).")
            ...
            if milvus_insert_payload: # This implies not skip_milvus_insert and faces were present and processed
                milvus_generated_ids = [] # Define it here
                try:
                    p_info(f"Attempting to insert {len(milvus_insert_payload)} new faces into Milvus collection '{EYE_OF_WEB_FACE_DATA_MILVUS_COLLECTION_NAME}'. Milvus will generate IDs.")
                    ...
                    if len(milvus_generated_ids) != len(milvus_insert_payload):
                         p_warn(f"Milvus insert count/ID mismatch. Expected {len(milvus_insert_payload)} IDs, got {len(milvus_generated_ids)}.")
                         conn.rollback() 
                         self.releaseConnection(conn, cursor)
                         return (False, "Milvus failed to return matching number of IDs for inserted faces.")
                    else:
                         p_log(f"Successfully inserted {len(milvus_generated_ids)} new faces into Milvus. Generated PKs from Milvus: {milvus_generated_ids}")
                    ...
                    for milvus_id in milvus_generated_ids:
                        try:
                            ...
                            if sql_face_id_result and sql_face_id_result["ID"]:
                                ...
                                conn.commit() 
                                p_log(f"Linked new Milvus ID {milvus_id} to new PostgreSQL EyeOfWebFaceID {pg_face_id}")
                            else:
                                p_error(f"Failed to retrieve PostgreSQL ID after inserting new MilvusRefID {milvus_id} into EyeOfWebFaceID.")
                                conn.rollback()
                                self.releaseConnection(conn, cursor)
                                return (False, "Failed to link new Milvus ID to PostgreSQL EyeOfWebFaceID.")
                        except psycopg2.Error as db_err:
                            conn.rollback() 
                            p_error(f"Database error inserting new MilvusRefID {milvus_id} into EyeOfWebFaceID: {db_err}")
                            traceback.print_exc()
                            self.releaseConnection(conn, cursor)
                            return (False, "Database error during new Milvus to SQL ID linking.")
                    
                    # Populate face_sql_ids_for_imagebasedmain with these newly created IDs
                    face_sql_ids_for_imagebasedmain = newly_created_pg_face_ids_from_milvus
                    p_log(f"Populated `face_sql_ids_for_imagebasedmain` with newly created PG FaceIDs: {face_sql_ids_for_imagebasedmain}")
            ...
            if faces and not skip_milvus_insert: 
                expected_face_count = len(faces) # Or len(milvus_insert_payload) if it was successfully built
                if len(face_sql_ids_for_imagebasedmain) != expected_face_count:
                    p_error(f"CRITICAL: Mismatch for new face processing. Input faces: {expected_face_count}, final linked SQL FaceIDs: {len(face_sql_ids_for_imagebasedmain)}. Data inconsistency. Rolling back.")
                    conn.rollback() 
                    self.releaseConnection(conn, cursor)
                    return (False, "Critical error in new face processing linkage. Transaction rolled back.")
            ...
            p_log(f"Final PostgreSQL FaceIDs to be associated with ImageBasedMain.FaceID: {face_sql_ids_for_imagebasedmain}")


            # --- Face Processing Sonu ---

            if not face_sql_ids_for_imagebasedmain and faces: 
                 p_warn("Although faces were provided in the input, no SQL Face IDs ended up in `face_sql_ids_for_imagebasedmain`. This might be due to an issue or if `skip_milvus_insert` was true but no existing faces were found for the hash initially.")
            ...
            if imageTitle is not None and str(imageTitle).strip():
                ...
                if imageTitle:
                    STATIC_SQL_COMMAND = """SELECT "ID" FROM "ImageTitleID" WHERE "Title" = %s"""
            ...
            if imagePath is not None and str(imagePath).strip():
                ...
                if imagePath:
                    ...
                    STATIC_SQL_COMMAND = """SELECT "ID" FROM "ImageUrlPathID" WHERE "Path" = %s"""
            ...
            if imagePathEtc is not None and str(imagePathEtc).strip():
                ...
                if imagePathEtc:
                    ...
                    STATIC_SQL_COMMAND = """SELECT "ID" FROM "ImageUrlEtcID" WHERE "Etc" = %s"""
            ...
            STATIC_CHECK_DATA_TUPLE = (
                protocol, baseDomain_return_id,
                urlPath_return_id, urlPath_return_id,
                urlPathEtc_return_id, urlPathEtc_return_id,
                imageProtocol, imageProtocol,
                imageDomain_return_id, imageDomain_return_id,
                imagePath_return_id, imagePath_return_id,
                imagePathEtc_return_id, imagePathEtc_return_id,
                imageTitle_return_id, imageTitle_return_id,
                image_return_id, image_return_id, # Bu ID ImageID tablosundan
            ...
            if not existing_main_record:
                ...
                face_ids_to_insert_update = face_sql_ids_for_imagebasedmain if face_sql_ids_for_imagebasedmain else None
                
                STATIC_INSERT_DATA_TUPLE = (
                    protocol, baseDomain_return_id, urlPath_return_id, urlPathEtc_return_id,
                    imageProtocol, imageDomain_return_id, imagePath_return_id, imagePathEtc_return_id,
                    imageTitle_return_id, image_return_id, 
                    face_ids_to_insert_update, # PostgreSQL array
                    riskLevel, category_return_id, imageHash_return_id, Source,
                ...
                conn.commit()
                p_log(f"New ImageBasedMain record created with ID: {inserted_main_id} and FaceIDs: {face_ids_to_insert_update}")
                self.releaseConnection(conn, cursor) # Release connection
                return (True, f"New ImageBasedMain record created with ID: {inserted_main_id}")
            else:
                # Kayıt zaten var. FaceID'yi güncelle.
                existing_main_id = existing_main_record["ID"]
                p_log(f"Existing ImageBasedMain record found with ID: {existing_main_id}.")
                ...
                cursor.execute("""SELECT "FaceID" FROM "ImageBasedMain" WHERE "ID" = %s""", (existing_main_id,))
                ...
                current_face_ids_in_db = set(current_face_ids_row["FaceID"] if current_face_ids_row and current_face_ids_row["FaceID"] else [])
                ...
                new_authoritative_face_ids_for_record = set(face_sql_ids_for_imagebasedmain if face_sql_ids_for_imagebasedmain else [])

                if new_authoritative_face_ids_for_record != current_face_ids_in_db:
                    p_info(f"Updating FaceID for ImageBasedMain ID {existing_main_id}. Old: {current_face_ids_in_db}, New authoritative list: {new_authoritative_face_ids_for_record}")
                    ...
                    final_face_ids_for_update = list(new_authoritative_face_ids_for_record) if new_authoritative_face_ids_for_record else None
                    cursor.execute(STATIC_UPDATE_SQL_COMMAND, (final_face_ids_for_update, current_timestamp_dt, existing_main_id))
                    conn.commit()
                    self.releaseConnection(conn, cursor) # Release connection
                    return (True, f"ImageBasedMain record ID {existing_main_id} updated with FaceIDs: {final_face_ids_for_update}.")
                else:
                    p_log(f"No change needed for FaceID in ImageBasedMain ID {existing_main_id}. FaceIDs remain: {current_face_ids_in_db}")
                    self.releaseConnection(conn, cursor) # Release connection
                    return (True, f"ImageBasedMain record ID {existing_main_id} already exists, FaceIDs unchanged.")
    ...
    def getAllDomains(self):
        ...
        cursor = conn.cursor(cursor_factory=DictCursor)
        try:
            STATIC_SQL_COMMAND = """SELECT DISTINCT "Domain" FROM "BaseDomainID" ORDER BY "Domain" """
            cursor.execute(STATIC_SQL_COMMAND)
    ...
    def getFaceDetailsWithImage(self, face_id: int):
        ...
        try:
            ...
            pg_cursor.execute('SELECT "MilvusRefID" FROM "EyeOfWebFaceID" WHERE "ID" = %s', (face_id,))
            ...
            if not milvus_ref_record or not milvus_ref_record["MilvusRefID"]:
                p_warn(f"No MilvusRefID found in EyeOfWebFaceID for PostgreSQL FaceID {face_id}. Cannot fetch from Milvus.")
            else:
                milvus_ref_id = milvus_ref_record["MilvusRefID"]
                result_data["milvus_ref_id"] = milvus_ref_id
                p_log(f"Found MilvusRefID: {milvus_ref_id} for PostgreSQL FaceID: {face_id}")

                # 3. Milvus'tan Vektör Özniteliklerini Çek
                milvus_collection = get_milvus_collection(EYE_OF_WEB_FACE_DATA_MILVUS_COLLECTION_NAME)
                if milvus_collection:
                    milvus_query_result = milvus_collection.query(
                        expr=f"id == {milvus_ref_id}",
                        output_fields=["face_gender", "face_age", "detection_score", "face_box", "landmarks_2d", "face_embedding_data"], # face_embedding_data eklendi
                    ...
                    if milvus_query_result:
                        ...
                        result_data["detection_score"] = milvus_entity.get("detection_score")
                        ...
                        if landmarks_flat_list and len(landmarks_flat_list) == 212:
                            try:
                                landmarks_2d_np = np.array(landmarks_flat_list, dtype=np.float32).reshape((106, 2))
                                result_data["landmarks_2d"] = landmarks_2d_np.tolist()
                            except ValueError as e:
                                p_error(f"Error reshaping landmarks_2d for MilvusRefID {milvus_ref_id}: {e}")
                        ...
                        if raw_embedding:
                            face_embedding_np = np.array(raw_embedding, dtype=np.float32)
                            if face_embedding_np.size == 512: # Beklenen boyut kontrolü
                                result_data["face_embedding"] = face_embedding_np.tolist() # veya np array olarak bırakılabilir
                            else:
                                p_warn(f"Fetched embedding for MilvusRefID {milvus_ref_id} has unexpected size: {face_embedding_np.size}. Expected 512.")
                                result_data["face_embedding"] = None
                        else:
                            result_data["face_embedding"] = None

                        p_log(f"Successfully fetched attributes (including embedding) from Milvus for MilvusRefID {milvus_ref_id}")
                        milvus_attributes_fetched = True
                    else:
                        p_warn(f"No data found in Milvus for MilvusRefID {milvus_ref_id} (linked to PostgreSQL FaceID {face_id}).")
                else:
                    p_warn(f"Failed to get Milvus collection for MilvusRefID {milvus_ref_id}. Skipping Milvus attribute fetch.")
            ...
            if meta_details_row:
                p_log(f"Successfully fetched metadata from PostgreSQL for FaceID {face_id}")
                ...
                result_data["detection_date"] = meta_details_row.get("detection_date").isoformat() if meta_details_row.get("detection_date") else None
                ...
                if result_data.get("protocol") and result_data.get("domain"):
                    ...
                    if result_data.get("url_path"): source_url += f'/{result_data["url_path"]}'
                    if result_data.get("url_etc"): source_url += f'{result_data["url_etc"]}'
                result_data["source_url"] = source_url
                
                full_image_url = None
                if result_data.get("image_protocol") and result_data.get("image_domain"):
                    full_image_url = f'{result_data["image_protocol"]}://{result_data["image_domain"]}'
                    if result_data.get("image_path"): full_image_url += f'/{result_data["image_path"]}'
                    if result_data.get("image_etc"): full_image_url += f'{result_data["image_etc"]}'
                result_data["full_image_url"] = full_image_url
            else:
                p_warn(f"No ImageBasedMain metadata found for PostgreSQL FaceID {face_id}.")

            if not milvus_attributes_fetched and not sql_metadata_fetched:
                p_error(f"No data found for FaceID {face_id} from either Milvus or PostgreSQL.")
                return None

            return result_data
        
        except MilvusException as me:
            p_error(f"MilvusException in getFaceDetailsWithImage (FaceID: {face_id}): {me}\n{traceback.format_exc()}")
            return None
        except psycopg2.Error as pe:
            p_error(f"Post psycopg2.Error in getFaceDetailsWithImage (FaceID: {face_id}): {pe}\n{traceback.format_exc()}")
            if pg_conn: pg_conn.rollback() # Normalde SELECT için rollback gerekmez ama emin olmak için.
            return None
        except Exception as e:
            p_error(f"Unexpected error in getFaceDetailsWithImage (FaceID: {face_id}): {e}\n{traceback.format_exc()}")
    ...
    def searchWhiteListFaces(self, face_name=None, institution=None, category=None, start_date=None, end_date=None):
        """
        ...
        """
        p_log(f"Searching WhiteListFaces (SQL only) with criteria: name={face_name}, inst={institution}, cat={category}, start={start_date}, end={end_date}")
        conn = self.connect()
        if not conn:
        ...
        try:
            ...
            if start_date:
                try:
                    ...
                    p_warn(f"Invalid start date format for searchWhiteListFaces: {start_date}")
            if end_date:
                 try:
                    ...
                    params.append(end_date)
                     ...
                     p_warn(f"Invalid end date format for searchWhiteListFaces: {end_date}")
            ...
            try:
                for face in faces:
                    ...
                    try:
                        ...
                        img_query = 'SELECT "FaceImage" FROM "WhiteListFaces" WHERE "ID" = %s'
                        img_cursor.execute(img_query, (pg_face_id,))
                        image_result = img_cursor.fetchone()
                        if image_result and image_result["FaceImage"]:
                            image_data = base64.b64encode(image_result["FaceImage"]).decode('utf-8')
                    except Exception as img_err:
                        p_error(f"Whitelist FaceImage fetch/encode error (ID: {pg_face_id}): {str(img_err)}")
                    ...
                    results.append({
                        ...
                        "face_description": face.get("FaceDescription"), # Şemaya eklendi varsayımıyla
                        # "institution": None, # Şemada yok
                        # "category": None,    # Şemada yok
                        # "gender": None, # Milvus'ta
                        # "age": None,    # Milvus'ta
                        # "detection_score": None, # Milvus'ta
                        "detection_date": face.get("DetectionDate").isoformat() if face.get("DetectionDate") else None,
                        "image_hash": face.get("FaceImageHash"),
                        "image_data": image_data,
                        "source": "whitelist"
                    })
            finally:
                if img_cursor: img_cursor.close()
            
            p_log(f"searchWhiteListFaces (SQL only) finished. Found {len(results)} faces.")
            return results
        
        except psycopg2.Error as pe:
            p_error(f"Post psycopg2.Error in searchWhiteListFaces: {pe}\n{traceback.format_exc()}")
            if conn: conn.rollback()
            return []
        except Exception as e:
            p_error(f"Unexpected error in searchWhiteListFaces: {e}\n{traceback.format_exc()}")
            if conn: conn.rollback()
            return []
        finally:
    ...
    def searchExternalFaces(self, face_name=None, start_date=None, end_date=None, alarm=None):
        """
        ...
        Returns:
            list: Eşleşen yüzlerin listesi (SQL meta verileri ve resim ile).
        """
        p_log(f"Searching ExternalFaceStorage (SQL only) with criteria: name={face_name}, start={start_date}, end={end_date}, alarm={alarm}")
        conn = self.connect()
        if not conn:
        ...
        try:
            ...
            if start_date:
                try:
                    ...
                    params.append(start_date)
                     ...
                     p_warn(f"Invalid start date format for searchExternalFaces: {start_date}")
            if end_date:
                 try:
                    datetime.datetime.strptime(end_date, '%Y-%m-%d')
                    where_clauses.append('"DetectionDate"::date <= %s::date')
                    params.append(end_date)
                     ...
                     p_warn(f"Invalid end date format for searchExternalFaces: {end_date}")
            if alarm is not None:
                ...
                if isinstance(alarm, bool):
                    ...
                    p_warn(f"Invalid type for alarm filter (expected bool): {type(alarm)}")
                ...
                for face in faces:
                    ...
                    try:
                        ...
                        if image_result and image_result["ImageData"]:
                            image_data = base64.b64encode(image_result["ImageData"]).decode('utf-8')
                    except Exception as img_err:
                        p_error(f"External ImageData fetch/encode error (ID: {pg_face_id}): {str(img_err)}")
            ...
            p_log(f"searchExternalFaces (SQL only) finished. Found {len(results)} faces.")
            return results
        
        except psycopg2.Error as pe:
            p_error(f"Post psycopg2.Error in searchExternalFaces: {pe}\n{traceback.format_exc()}")
            if conn: conn.rollback()
            return []
        except Exception as e:
            p_error(f"Unexpected error in searchExternalFaces: {e}\n{traceback.format_exc()}")
            if conn: conn.rollback()
            return []
        finally:
            # Ana cursor ve bağlantı kapatma
    ...
    def searchEgmArananlar(self, face_name=None, organizer=None, organizer_level=None, start_date=None, end_date=None):
        """
        ...
        try:
            ...
            img_cursor = conn.cursor(cursor_factory=DictCursor) # Resim için ayrı cursor
            try:
                while True:
                    ...
                    try:
                        ...
                        img_query = "SELECT \"ImageData\" FROM \"EgmArananlar\" WHERE \"ID\" = %s"
                        img_cursor.execute(img_query, (face["ID"],))
                        image_result = img_cursor.fetchone()
                        if image_result and image_result["ImageData"]:
                            image_data = base64.b64encode(image_result["ImageData"]).decode('utf-8')
                    except Exception as img_err:
                        print(f"EGM Resim verisi alınırken hata (ID: {face['ID']}): {str(img_err)}")
                    ...
                    results.append({
                        ...
                        "detection_date": face["DetectionDate"].isoformat() if face["DetectionDate"] else None,
        ...
        try:
            ...
            if face_data:
                ...
                if result['face_landmarks'] is not None:
                    result['face_landmarks'] = numpy.frombuffer(result['face_landmarks'], dtype=numpy.float32).reshape(-1, 2)
                
                # Face encoding verileri numpy dizisine dönüştür
                if result['face_encoding'] is not None:
                    result['face_encoding'] = numpy.frombuffer(result['face_encoding'], dtype=numpy.float32)
                
                # Yüz görseli varsa base64 formatına dönüştür
                if result['face_image'] is not None:
                    result['image_data'] = base64.b64encode(result['face_image']).decode('utf-8')
    ...
    def getWhitelistFaceDetailsWithLandmarks(self, face_id):
        ...
        try:
            ...
            if face_data:
                ...
                return {
                    'success': False,
                    'message': 'Beyaz liste yüzü bulunamadı'
                }
        except Exception as e:
            return {
                'success': False,
    ...
    def getEgmFaceDetailsWithLandmarks(self, face_id):
        ...
        try:
            ...
            if face_data:
                ...
                return {
                    'success': False,
                    'message': 'EGM aranan yüzü bulunamadı'
                }
        except Exception as e:
            return {
                'success': False,
    ...
    def get_embedding_by_id(self, face_id: int) -> np.ndarray | None:
        ...
        try:
            ...
            pg_conn = self.connect()
            if not pg_conn:
                p_error(f"Database connection failed for get_embedding_by_id (FaceID: {face_id}).")
                return None
            pg_cursor = pg_conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
            
            sql_get_milvus_ref = 'SELECT "MilvusRefID" FROM "EyeOfWebFaceID" WHERE "ID" = %s'
            pg_cursor.execute(sql_get_milvus_ref, (face_id,))
            milvus_ref_record = pg_cursor.fetchone()

            if not milvus_ref_record or milvus_ref_record["MilvusRefID"] is None:
                p_warn(f"No MilvusRefID found in EyeOfWebFaceID for PostgreSQL FaceID {face_id}. Cannot fetch embedding from Milvus.")
                return None
            
            milvus_ref_id = milvus_ref_record["MilvusRefID"]
            p_log(f"Found MilvusRefID: {milvus_ref_id} for PostgreSQL FaceID: {face_id}. Querying Milvus.")

            # 2. Milvus'tan embedding verisini çek
            milvus_collection = get_milvus_collection(EYE_OF_WEB_FACE_DATA_MILVUS_COLLECTION_NAME)
            if not milvus_collection:
                p_error(f"Failed to get Milvus collection '{EYE_OF_WEB_FACE_DATA_MILVUS_COLLECTION_NAME}' for FaceID: {face_id} (MilvusRefID: {milvus_ref_id}).")
                return None
            ...
            if milvus_results:
                ...
                if raw_embedding:
                    ...
                    if embedding_data.size == 512: # Beklenen boyut kontrolü
                        p_log(f"Successfully fetched embedding from Milvus for MilvusRefID: {milvus_ref_id}")
                    else:
                        p_warn(f"Fetched embedding for MilvusRefID {milvus_ref_id} has unexpected size: {embedding_data.size}. Expected 512.")
                        embedding_data = None # Boyut yanlışsa None döndür
                else:
                    p_warn(f"'face_embedding_data' field not found or is null in Milvus for MilvusRefID: {milvus_ref_id}.")
            else:
                p_warn(f"No data found in Milvus for MilvusRefID {milvus_ref_id} (linked to PostgreSQL FaceID {face_id}).")
        
        except MilvusException as me:
            p_error(f"MilvusException in get_embedding_by_id (FaceID: {face_id}, MilvusRefID: {milvus_ref_id if 'milvus_ref_id' in locals() else 'N/A'}): {me}")
            traceback.print_exc()
            embedding_data = None
        except psycopg2.Error as pe:
            p_error(f"PostgreSQL error in get_embedding_by_id (FaceID: {face_id}): {pe}")
            traceback.print_exc()
            if pg_conn: pg_conn.rollback() # Genelde SELECT için gerekmez ama hata durumunda.
            embedding_data = None
        except Exception as e:
            ...
            if pg_conn: pg_conn.close() # self.releaseConnection yerine doğrudan close kullanılabilir.
            
        return embedding_data

    def searchFaces(self, domain=None, risk_level=None, category=None, start_date=None, end_date=None, page=1, per_page=50):
        """
        ...
        """
        p_log(f"Searching faces with criteria: domain={domain}, risk={risk_level}, cat={category}, start={start_date}, end={end_date}, page={page}, per_page={per_page}")
        results = []
        total_count = 0
        pg_conn = None
        pg_cursor = None
        ...
        try:
            ...
            if start_date:
                try:
                    ...
                    p_warn(f"Geçersiz başlangıç tarihi formatı: {start_date}. Filtre uygulanmayacak.")
            if end_date:
                 try:
                    datetime.datetime.strptime(end_date, '%Y-%m-%d')
                    filter_conditions.append('m."DetectionDate"::date <= %s::date')
                    params.append(end_date)
                     ...
                     p_warn(f"Geçersiz bitiş tarihi formatı: {end_date}. Filtre uygulanmayacak.")
            ...
            try:
                ...
                if per_page > 100: per_page = 100 # Max per_page (isteğe bağlı)
            ...
            p_log(f"Fetched {len(sql_results_for_page)} records from SQL for page {page}")

            # HATA AYIKLAMA: İlk birkaç SQL sonucunu kontrol et
            if sql_results_for_page:
                p_log(f"DEBUG: Sample SQL results - First record: pg_face_id={sql_results_for_page[0].get('pg_face_id')}, milvus_ref_id={sql_results_for_page[0].get('milvus_ref_id')}")
            ...
            milvus_ref_ids_to_fetch = [row["milvus_ref_id"] for row in sql_results_for_page if row["milvus_ref_id"]]
            milvus_attributes_map = {}

            # HATA AYIKLAMA: MilvusRefID'leri logla
            p_log(f"DEBUG: Found {len(milvus_ref_ids_to_fetch)} MilvusRefIDs in SQL results")
            if not milvus_ref_ids_to_fetch:
                p_error("ERROR: No MilvusRefIDs found in SQL results - this is a critical database synchronization issue!")
            ...
            if milvus_ref_ids_to_fetch:
                ...
                if milvus_collection:
                    ...
                    p_log(f"Querying Milvus for {len(milvus_ref_ids_to_fetch)} MilvusRefIDs: {expr_milvus_ids[:200]}...")
                    milvus_query_results = milvus_collection.query(
                        expr=f"id in [{expr_milvus_ids}]",
                        output_fields=["id", "face_gender", "face_age", "detection_score", "face_box", "landmarks_2d"],
                    )
                    for item in milvus_query_results:
                        milvus_attributes_map[item["id"]] = item # Milvus ID'sine göre map yap
                    p_log(f"Fetched {len(milvus_attributes_map)} attribute sets from Milvus.")
                    
                    # HATA AYIKLAMA: MilvusRefID'ler ile Milvus sonuçlarını karşılaştır
                    missing_ids = [mid for mid in milvus_ref_ids_to_fetch if mid not in milvus_attributes_map]
                    if missing_ids:
                        p_error(f"ERROR: {len(missing_ids)} MilvusRefIDs could not be found in Milvus: {missing_ids[:5]}...")
                else:
                    p_error("CRITICAL: Failed to get Milvus collection for searchFaces attribute fetch.")
            ...
            for row in sql_results_for_page:
                ...
                if milvus_ref_id and milvus_ref_id in milvus_attributes_map:
                    ...
                    face_data["facebox"] = milvus_attrs.get("face_box") # Liste olarak bekleniyor
                    
                    landmarks_flat = milvus_attrs.get("landmarks_2d")
                    if landmarks_flat and len(landmarks_flat) == 212:
                        try:
                            face_data["landmarks_2d"] = np.array(landmarks_flat, dtype=np.float32).reshape((106,2)).tolist()
                        except ValueError as e:
                            p_error(f"Error reshaping landmarks for MilvusID {milvus_ref_id} in searchFaces: {e}")
                            face_data["landmarks_2d"] = None
                    else:
                        face_data["landmarks_2d"] = None
                else:
                    # Milvus'tan öznitelik gelmediyse veya MilvusRefID yoksa alanları varsayılan değerlere ayarla
                    p_warn(f"No Milvus attributes found for face_id={pg_face_id}, milvus_id={milvus_ref_id}")
                    face_data["gender"] = "Bilinmiyor"
                    face_data["age"] = 0
                ...
                if face_data.get("source_protocol") and face_data.get("source_domain"):
                    source_url = f'{face_data["source_protocol"]}://{face_data["source_domain"]}'
                    if face_data.get("source_url_path"): source_url += f'/{face_data["source_url_path"]}'
                    if face_data.get("source_url_etc"): source_url += f'{face_data["source_url_etc"]}'
                face_data["source_url"] = source_url
                
                full_image_url = None
                if face_data.get("image_protocol") and face_data.get("image_domain"):
                    full_image_url = f'{face_data["image_protocol"]}://{face_data["image_domain"]}'
                    if face_data.get("image_path"): full_image_url += f'/{face_data["image_path"]}'
                    if face_data.get("image_etc"): full_image_url += f'{face_data["image_etc"]}'
                face_data["full_image_url"] = full_image_url
                
                # Tarih formatını düzelt
                if face_data.get('detection_date'):
                    face_data['detection_date'] = face_data['detection_date'].isoformat()
                ...
                image_content_id = face_data.get('image_id') # Bu ImageID tablosunun ID'si
                if image_content_id:
                    ...
                    if success and binary_data:
                        try:
                            ...
                            if isinstance(decompressed_binary, bytes):
                                image_b64 = base64.b64encode(decompressed_binary).decode('utf-8')
                                face_data['image_data_base64'] = image_b64
                                face_data['image_data'] = image_b64  # Template için image_data alanı
                                face_data['use_default_image'] = False  # Resim bulundu
                                face_data['image_mime_type'] = 'image/png'  # Decompress sonrası PNG
                        except Exception as encode_err:
                            p_error(f"Error decompressing/encoding image_id {image_content_id} for searchFaces (pg_face_id: {pg_face_id}): {encode_err}")
                
                results.append(face_data)
            
            # HATA AYIKLAMA: Sonuçları logla
            for idx, face_data in enumerate(results[:3]):  # İlk 3 sonuç için
                p_log(f"DEBUG: Result #{idx+1} - id: {face_data.get('id')}, gender: {face_data.get('gender')}, age: {face_data.get('age')}, has_image: {'Evet' if not face_data.get('use_default_image') else 'Hayır'}")
            
            p_log(f"searchFaces finished for page {page}. Returning {len(results)} faces. Total count: {total_count}")
            ...
            p_error(f"Post psycopg2.Error in searchFaces: {pe}\n{traceback.format_exc()}")
            if pg_conn: pg_conn.rollback()
            return [], 0
        except Exception as e:
    ...
    def findSimilarWhiteListFaces(self, face_embedding, threshold=0.6, algorithm='cosine', use_cuda=False):
        ...
        img_cursor = None
        try:
            embedding_str = str(face_embedding.tolist()) # <<< YENİ: String formatına çevir
            ...
            cursor.execute(query, (embedding_str, embedding_str, embedding_str, distance_threshold))
            similar_raw = cursor.fetchall() # Önce benzerleri al

            p_log(f"pgvector (Whitelist): Eşik ({threshold} / mesafe < {distance_threshold:.4f}) üzerinde {len(similar_raw)} yüz bulundu.")
                ...
                for face in similar_raw:
                    ...
                    try:
                        ...
                        if image_result and image_result["FaceImage"]:
                         ...
                         p_error(f"Whitelist Resim verisi alınırken/encode edilirken hata (ID: {face_id}): {str(img_err)}")

                    face_dict = {
                        "id": face_id,
                        "similarity": float(face["similarity"]),
                        "face_name": face["FaceName"],
                        "face_description": face.get("FaceDescription"), # .get() kullan
                        # "institution": face.get("Institution"), # Yok
                        # "category": face.get("Category"),       # Yok
                        "gender": face["FaceGender"],
                        "age": face["FaceAge"],
                        "detection_score": float(face["DetectionScore"]) if face["DetectionScore"] is not None else 0.0,
                        "insertion_date": face["DetectionDate"].isoformat() if face["DetectionDate"] else None, # Tarih sütunu adı düzeltildi
                        "image_data": image_data,
                        "source": "whitelist"
                    }
    ...
    def findSimilarFacesWithImages(self, face_embedding: np.ndarray, threshold=0.6, limit=100, algorithm='cosine', use_cuda=False):
        """
        ...
        """
        p_log(f"Starting findSimilarFacesWithImages with threshold: {threshold}, Milvus limit: {limit}")
        ...
        try:
            ...
            milvus_collection = get_milvus_collection(EYE_OF_WEB_FACE_DATA_MILVUS_COLLECTION_NAME)
            if not milvus_collection:
                p_error("Failed to get Milvus collection for findSimilarFacesWithImages.")
                return []
            ...
            search_params = {
                "metric_type": "COSINE", 
                "params": {"ef": max(1200, limit * 1.2)},  # ef değeri limit'ten en az %20 büyük ve minimum 1200 olmalı
            }

            p_info(f"Searching Milvus collection \'{EYE_OF_WEB_FACE_DATA_MILVUS_COLLECTION_NAME}\' with limit {limit} and ef={search_params['params']['ef']}...")
            milvus_results = milvus_collection.search(
                ...
                output_fields=["id", "face_gender", "face_age", "detection_score", "face_box", "landmarks_2d", "detection_date_ts"],
                consistency_level="Strong" # Veya uygulamanız için uygun olan
            )
            p_log(f"Milvus search returned {len(milvus_results[0]) if milvus_results else 0} raw hits.")
            ...
            processed_face_pg_ids = set() # Aynı pg_face_id'nin birden fazla Milvus ID'si ile eşleşmesini önlemek için (eğer mümkünse)

            distance_threshold = 1 - threshold # Eşik değerini mesafeye çevir
            p_log(f"Converted similarity threshold {threshold} to distance threshold {distance_threshold:.4f}")
            ...
            for hits_for_query in milvus_results:
                for hit in hits_for_query:
                    ...
                    if similarity_score >= threshold:
                        p_log(f"Milvus Hit: MilvusID={milvus_id}, Distance={distance:.4f} - Below distance threshold {distance_threshold:.4f} (Original Sim Threshold: {threshold})")
                        ...
                        milvus_face_box_list = entity.get("face_box") # Liste olarak bekleniyor
                        milvus_landmarks_flat_list = entity.get("landmarks_2d") # Düz liste olarak bekleniyor (212 eleman)
                        ...
                        if milvus_landmarks_flat_list and len(milvus_landmarks_flat_list) == 212:
                            try:
                                landmarks_2d_np = np.array(milvus_landmarks_flat_list, dtype=np.float32).reshape((106, 2))
                            except ValueError as e:
                                p_error(f"Error reshaping landmarks_2d for MilvusID {milvus_id}: {e}. Data: {milvus_landmarks_flat_list[:10]}...")
                                landmarks_2d_np = None
                        
                        facebox_data = milvus_face_box_list # Zaten liste olmalı

                        # Milvus ID'sini kullanarak PostgreSQL'den EyeOfWebFaceID.ID'yi al
                        pg_cursor.execute('SELECT "ID" FROM "EyeOfWebFaceID" WHERE "MilvusRefID" = %s', (milvus_id,))
                        pg_face_record = pg_cursor.fetchone()

                        if not pg_face_record:
                            p_warn(f"No corresponding PostgreSQL EyeOfWebFaceID found for MilvusID {milvus_id}. Skipping.")
                            continue
                        
                        pg_face_id = pg_face_record["ID"]

                        if pg_face_id in processed_face_pg_ids:
                            p_log(f"PostgreSQL FaceID {pg_face_id} (from MilvusID {milvus_id}) already processed. Skipping duplicate enrichment.")
                            continue
                        ...
                        if not face_details_row:
                            p_warn(f"No ImageBasedMain details found for PostgreSQL FaceID {pg_face_id} (from MilvusID {milvus_id}). Skipping.")
                            continue
                        ...
                        similar_faces_output.append({
                            ...
                            "image_id": face_details_row.get('image_id'), # ImageID tablosunun ID'si
                            ...
                            "detection_date": face_details_row.get('detection_date').isoformat() if face_details_row.get('detection_date') else None,
                            "facebox": facebox_data, # Milvus'tan gelen liste
                            "landmarks_2d": landmarks_2d_np.tolist() if landmarks_2d_np is not None else None, # (106,2) formatında liste
                            "image_info": image_info_dict,
                            "source_table": face_details_row.get('source_table_name', 'EyeOfWebFaceDataMilvus') # Kaynağı belirtmek için
                        })
                    else:
                        p_log(f"Milvus Hit: MilvusID={milvus_id}, Similarity={similarity_score:.4f} - Below threshold {threshold}. Skipping.")
            ...
            p_log(f"findSimilarFacesWithImages finished. Returning {len(similar_faces_output)} processed faces.")
            return similar_faces_output

        except MilvusException as me:
            p_error(f"MilvusException in findSimilarFacesWithImages: {me}\n{traceback.format_exc()}")
            return []
        except psycopg2.Error as pe:
            p_error(f"Post psycopg2.Error in findSimilarFacesWithImages: {pe}\n{traceback.format_exc()}")
            if pg_conn: pg_conn.rollback()
            return []
        except Exception as e:
            p_error(f"Unexpected error in findSimilarFacesWithImages: {e}\n{traceback.format_exc()}")
            if pg_conn: pg_conn.rollback() # Emin olmak için
            return []
        finally:
            if pg_cursor:
                pg_cursor.close()
    ...
    def findSimilarFaces(self, face_embedding: np.ndarray, threshold=0.6, limit=100, algorithm='cosine', use_cuda=False): # Added limit, algorithm/use_cuda are for API compatibility but Milvus handles internally
        """
        ...
        """
        p_log(f"Starting Milvus-based findSimilarFaces with threshold: {threshold}, Milvus search limit: {limit}")
        similar_faces_output = []
        pg_conn = None
        pg_cursor = None
        ...
        try:
            ...
            # 2. Milvus'ta Benzerlik Araması
            search_vector = [np.array(face_embedding, dtype=np.float32).tolist()] # Ensure correct format for Milvus
            ...
            p_log(f"Milvus search returned {len(milvus_results[0]) if milvus_results and milvus_results[0] else 0} raw hits.")
            ...
            pg_conn = self.connect()
            if not pg_conn:
                p_error("Database connection failed for enriching Milvus results in findSimilarFaces.")
                return []
            ...
            for hit in milvus_results[0]: 
                milvus_id = hit.id
                similarity_score_from_milvus = hit.distance # Using Milvus COSINE distance as similarity score per user spec

                if similarity_score_from_milvus >= threshold:
                    p_log(f"Milvus Hit: MilvusID={milvus_id}, Reported Distance (used as Similarity)={similarity_score_from_milvus:.4f} >= threshold {threshold}")
                    entity = hit.entity 
                    ...
                    if pg_face_id in processed_pg_face_ids: 
                        p_log(f"PostgreSQL FaceID {pg_face_id} (from MilvusID {milvus_id}) already processed. Skipping.")
                        continue
                    ...
                    current_face_data = {
                        ...
                        "detection_score": float(entity.get("detection_score")) if entity.get("detection_score") is not None else None,
                        "domain": None,
                        "detection_date": None,
                        "risk_level": None,
                        "category": None
                    ...
                    if face_details_row:
                        current_face_data.update({
                            ...
                            "detection_date": face_details_row["DetectionDate"].isoformat() if face_details_row["DetectionDate"] else None,
                            "risk_level": face_details_row["RiskLevel"],
                            "category": face_details_row["Category"]
                        })
                    else:
                        p_warn(f"No ImageBasedMain details found for PostgreSQL FaceID {pg_face_id} (from MilvusID {milvus_id}).")
                    similar_faces_output.append(current_face_data)
                else:
                    p_log(f"Milvus Hit: MilvusID={milvus_id}, Reported Distance (used as Similarity)={similarity_score_from_milvus:.4f} < threshold {threshold}. Skipping.")

            similar_faces_output.sort(key=lambda x: x['similarity'], reverse=True)
            p_log(f"findSimilarFaces (Milvus-based) finished. Returning {len(similar_faces_output)} faces.")
            return similar_faces_output

        except MilvusException as me:
            p_error(f"MilvusException in findSimilarFaces: {me}\n{traceback.format_exc()}")
            return []
        except psycopg2.Error as pe:
            p_error(f"PostgreSQL error in findSimilarFaces: {pe}\n{traceback.format_exc()}")
            if pg_conn: pg_conn.rollback()
            return []
        except Exception as e:
            p_error(f"Unexpected error in findSimilarFaces: {e}\n{traceback.format_exc()}")
            if pg_conn: pg_conn.rollback() 
            return []
        finally:
            if pg_cursor: pg_cursor.close()
            if detail_cursor: detail_cursor.close()
    ...
    def get_milvus_face_attributes(self, collection_name: str, pg_face_id: int) -> dict:
        ...
        """
        p_log(f"Retrieving Milvus attributes for PostgreSQL FaceID: {pg_face_id} from collection: {collection_name}")
        pg_conn = None
        pg_cursor = None
        ...
        try:
            ...
            if milvus_ref_id is None:
                ...
                pg_conn = self.connect()
                if not pg_conn:
                    p_error(f"Database connection failed for get_milvus_face_attributes (FaceID: {pg_face_id}).")
                    return None
                pg_cursor = pg_conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
                ...
                if not milvus_ref_record or milvus_ref_record["MilvusRefID"] is None:
                    p_warn(f"No MilvusRefID found in EyeOfWebFaceID for PostgreSQL FaceID {pg_face_id}.")
                    return None
            ...
            p_log(f"Found MilvusRefID: {milvus_ref_id} for PostgreSQL FaceID: {pg_face_id}. Querying Milvus.")

            # 2. Milvus'tan veri çek (önbellekten koleksiyon al)
            milvus_collection = self._get_cached_milvus_collection(collection_name)
            if not milvus_collection:
                p_error(f"Failed to get Milvus collection '{collection_name}' for FaceID: {pg_face_id} (MilvusRefID: {milvus_ref_id}).")
                return None
            ...
            milvus_results = milvus_collection.query(
                expr=query_expr,
                output_fields=["face_embedding_data", "face_box", "landmarks_2d", "face_gender", "face_age", "detection_score", "detection_date_ts"],
                limit=1 # ID unique olmalı
            )

            if not milvus_results:
                p_warn(f"No data found in Milvus (collection: {collection_name}) for MilvusRefID: {milvus_ref_id}")
                return None
            ...
            p_log(f"Successfully retrieved Milvus attributes for FaceID: {pg_face_id} (MilvusRefID: {milvus_ref_id}).")
            return attributes
        
        except MilvusException as me:
            p_error(f"MilvusException in get_milvus_face_attributes (FaceID: {pg_face_id}): {me}")
            traceback.print_exc()
            return None
        except psycopg2.Error as pe:
            p_error(f"PostgreSQL error in get_milvus_face_attributes (FaceID: {pg_face_id}): {pe}")
            traceback.print_exc()
            if pg_conn: pg_conn.rollback()
            return None
        except Exception as e:
            p_error(f"Unexpected error in get_milvus_face_attributes (FaceID: {pg_face_id}): {e}")
            traceback.print_exc()
            return None
        finally:
            if pg_cursor: pg_cursor.close()
    ...
    def find_similar_face_ids_in_milvus(self, collection_name: str, target_vector: list, distance_threshold: float, exclude_id: int = None, limit: int = 1000) -> list:
        """
        ...
        """
        p_log(f"Searching Milvus for similar faces with distance threshold: {distance_threshold}, limit: {limit}")
        pg_conn = None
        pg_cursor = None
        similar_pg_face_ids = []
        ...
        try:
            ...
            milvus_collection = self._get_cached_milvus_collection(collection_name)
            if not milvus_collection:
                p_error(f"Failed to get Milvus collection '{collection_name}' for similarity search.")
                return []
            ...
            if exclude_id is not None:
                ...
                if exclude_milvus_id is None:
                    ...
                    pg_conn = self.connect()
                    if not pg_conn:
                        p_error(f"Database connection failed for find_similar_face_ids_in_milvus.")
                        return []
                    pg_cursor = pg_conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
                    
                    pg_cursor.execute('SELECT "MilvusRefID" FROM "EyeOfWebFaceID" WHERE "ID" = %s', (exclude_id,))
                ...
                if exclude_milvus_id:
                    p_log(f"Will exclude MilvusID: {exclude_milvus_id} (PostgreSQL FaceID: {exclude_id}) from search results.")
            ...
            search_vector = [target_vector]  # Milvus search_vector formatı: liste içinde liste
            p_log(f"Searching in Milvus collection '{collection_name}' with limit {limit} and ef={search_params['params']['ef']}...")
            ...
            for hit in milvus_results[0]:
                ...
                if distance <= distance_threshold:
                    ...
                    p_log(f"Skipping MilvusID {milvus_id} with distance {distance} > threshold {distance_threshold}")
            ...
            if pg_conn is None or pg_cursor is None:
                pg_conn = self.connect()
                if not pg_conn:
                    p_error("Database connection failed for Milvus ID to PostgreSQL ID conversion.")
                    return []
                pg_cursor = pg_conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
    ...
    def get_batch_milvus_face_attributes(self, collection_name: str, pg_face_ids: list) -> dict:
        """
        ...
        try:
            ...
            if missing_ids:
                p_log(f"Önbellekte bulunmayan {len(missing_ids)} FaceID için MilvusRefID'leri alınıyor...")
            ...
            for face_id in pg_face_ids:
                ...
                milvus_results = milvus_collection.query(
                    expr=query_expr,
                    output_fields=["face_embedding_data", "face_box", "landmarks_2d", "face_gender", "face_age", "detection_score"],
                    limit=1
                )
                ...
                if milvus_results:
                    ...
                    p_warn(f"FaceID {face_id} (MilvusRefID {milvus_ref_id}) için Milvus'ta veri bulunamadı.")
            
            p_info(f"Toplu Milvus verileri başarıyla alındı: {len(results)}/{len(pg_face_ids)} yüz")
            return results

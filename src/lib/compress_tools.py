...
def compress_image(image_binary):
    ...
    # Resize image
    scale_ratio = 0.5 # You can adjust this ratio if needed
...
def decompress_image(compressed_binary):
    ...
    if pil_img.mode == 'RGB':
         ...
         img_array_bgr = cv2.cvtColor(img_array, cv2.COLOR_RGBA2BGR) # Convert RGBA to BGR
    elif pil_img.mode == 'L': # Grayscale
         img_array_bgr = cv2.cvtColor(img_array, cv2.COLOR_GRAY2BGR) # Convert grayscale to BGR
    else:
         # Fallback or raise error for unsupported modes
         print(f"Warning: Unexpected PIL image mode '{pil_img.mode}'. Attempting direct use.")
...
def format_size(bytes_val):
    ...
    if bytes_val < 0: return "N/A" # Handle potential errors
...
if __name__ == "__main__":
    ...
    try:
        ...
        original_img_rgb = cv2.cvtColor(original_img_cv, cv2.COLOR_BGR2RGB) # For matplotlib
        ...
        resized_img_rgb = cv2.cvtColor(resized_img_cv, cv2.COLOR_BGR2RGB) # For matplotlib
        ...
        print(f"Estimated Resized Size (WebP Q=90): {format_size(resized_webp_q90_size)}")
        ...
        print(f"Final Compressed Size (Resized -> WebP Q90 -> Zlib): {format_size(compressed_size)}")
        ...
        plt.title(f"Resized Image (Ratio: {scale_ratio})\n"

...
class PDFReport(FPDF):
    def header(self):
        ...
        if os.path.exists(LOGO_PATH):
            try:
                self.image(LOGO_PATH, x=self.l_margin, y=logo_y_pos, w=logo_width, h=logo_height)
                ...
                self.cell(logo_width, 10, 'Logo Yok', 1, 0, 'C') # Hata durumunda çerçeveli hücre
                logo_height = 10
        else:
        ...
        available_width = self.w - self.l_margin - self.r_margin - logo_width - 5 # 5mm boşluk
        ...
        header_bottom_y = max(logo_y_pos + logo_height + 2, title_y_pos + 10) # Çizgiyi daha yukarı çektik
    ...
    def create_table_cell_multiline(self, label, value, x_position, label_width, value_width):
        ...
        is_multiline = '\n' in value_str or len(value_str) > 45 or is_url # Eşiği düşürdük
        ...
        if is_multiline:
            ...
            line_height = 3.5 if is_url else 5 # URL için satır yüksekliğini daha da azalttık
    ...
    def add_comparison_info(self, comparison_info):
        ...
        self.create_table_cell_multiline("Sonuç", message, x_start, label_width, value_width)
        self.ln(1)
        ...
        if score is not None:
            ...
            self.create_table_cell("Benzerlik Skoru", score_str, x_start, label_width, value_width)
        ...
        if threshold is not None:
            ...
            self.create_table_cell("Kullanılan Eşik", threshold_str, x_start, label_width, value_width)
    ...
    def add_side_by_side_faces(self, face1_b64, face2_b64):
        ...
        self.ln(2)
        self.set_font(FONT_FAMILY, 'B', 11)
        self.cell(0, 7, "Karşılaştırılan Yüzler", border='B', ln=1, align='C') # Centered title
        self.ln(3)
        ...
        try:
            ...
            self.image(img_stream1, x=start_x, y=current_y, w=FACE_CUTOUT_SIZE, h=FACE_CUTOUT_SIZE, type='JPEG')
        ...
        try:
            ...
            self.image(img_stream2, x=start_x + FACE_CUTOUT_SIZE + gap, y=current_y, w=FACE_CUTOUT_SIZE, h=FACE_CUTOUT_SIZE, type='JPEG')
            self.set_xy(start_x + FACE_CUTOUT_SIZE + gap, current_y + FACE_CUTOUT_SIZE + 1)
            self.set_font(FONT_FAMILY, '', 8)
    ...
    def add_comparison_face_detail(self, face_data):
        ...
        if face_cropped_b64:
            try:
                ...
                self.image(img_stream, x=image_x, y=current_y, w=FACE_CUTOUT_SIZE, h=FACE_CUTOUT_SIZE, type='JPEG')
            except Exception as e:
        ...
        details_to_show = [
            {"label": "Kaynak Dosya", "value": face_data.get('title', 'N/A').split('(')[-1].replace(')', '') }, # Extract filename
            {"label": "Tespit Skoru", "value": f"{face_data.get('score'):.2f}" if face_data.get('score') is not None else 'N/A'},
            {"label": "Cinsiyet", "value": face_data.get('gender', 'N/A')},
            {"label": "Yaş", "value": face_data.get('age') if face_data.get('age') is not None else 'N/A'}
        ]

        for detail in details_to_show:
             self.create_table_cell(detail["label"], detail["value"], text_x, label_width, value_width)
    ...
    def add_image_details(self, image_data, is_pre_boxed=False):
        ...
        image_data_b64 = image_data.get('image_data_b64') # Base64 verisi için yeni alan
        ...
        if image_data_b64:
            ...
            try:
                ...
                except: pass # Ignore errors in format detection
            ...
            if original_image_url:
                try:
                    ...
                    print(f"URL ayrıştırılırken hata: {original_image_url} - {parse_err}")
                    image_url_to_try = original_image_url

            urls_to_attempt = []
            if image_url_to_try: urls_to_attempt.append(image_url_to_try)
            ...
            if not urls_to_attempt:
                ...
                for attempt_url in urls_to_attempt:
                    if not attempt_url: continue
                    try:
                        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'}
                        response = requests.get(attempt_url, stream=True, timeout=10, headers=headers)
                        response.raise_for_status()
                        content_type = response.headers.get('content-type')
                        if content_type and content_type.lower() in ['image/jpeg', 'image/png', 'image/gif']:
                            try:
                                img_pil = Image.open(io.BytesIO(response.content))
                                img_pil = img_pil.convert("RGB")
                                img_stream = io.BytesIO()
                                img_format = 'JPEG' if content_type.lower() == 'image/jpeg' else 'PNG'
                                ...
                                if image_load_error is None: image_load_error = current_error
                                img_pil = None
                                img_stream = None
                                continue
                        else:
             ...
             image_load_error = "Görüntülenecek resim verisi (URL veya Base64) sağlanmadı."

        # --- İçerik Bloğu Oluşturma --- 
        # Sayfa sonu kontrolü
        if self.get_y() + MAX_IMAGE_HEIGHT + 20 > self.page_break_trigger:  # 20mm güvenlik payı
            self.add_page()
            self.add_watermark()
        ...
        if not is_pre_boxed and img_pil and facebox and isinstance(facebox, (list, tuple)) and len(facebox) == 4:
            try:
        ...
        if img_stream and content_type_for_fpdf:
            try:
                self.image(img_stream, x=image_x, y=block_start_y, w=MAX_IMAGE_WIDTH, h=MAX_IMAGE_HEIGHT, type=content_type_for_fpdf)
                image_added = True
        ...
        details = [
            ...
            {"label": "Resim Kaynağı", "value": original_image_url if original_image_url else ('Base64 Veri' if image_data_b64 else 'N/A'), "is_url": bool(original_image_url)},
            {"label": "Kaynak URL", "value": format_url(image_data.get('source_url', 'N/A')), "is_url": True},
        ...
        if 'score' in image_data and image_data.get('score') is not None:
            details.append({"label": "Tespit Skoru", "value": f"{image_data.get('score'):.2f}"})
            
        if ('gender' in image_data and image_data.get('gender') is not None) and \
           ('age' in image_data and image_data.get('age') is not None):
            details.append({"label": "Cinsiyet/Yaş", "value": f"{image_data.get('gender')} / {image_data.get('age')}"})
        ...
        block_height = max(block_end_y - block_start_y, MAX_IMAGE_HEIGHT if image_added else 0)
...
def generate_pdf_report(search_type, username, search_results, comparison_info=None):
    """
    ...
    try:
        ...
        if not os.path.exists(font_path):
            print(f"Uyarı: Font dosyası bulunamadı: {font_path}. Standart font kullanılacak.")
        ...
        # Sayfa boyutunu biraz değiştir (yeterli içerik alanı için)
        pdf.add_page()
        ...
        if comparison_info:
            ...
            face1_crop_b64 = search_results[0].get('face_cropped_b64') if len(search_results) > 0 else None
            face2_crop_b64 = search_results[1].get('face_cropped_b64') if len(search_results) > 1 else None
        ...
        if not search_results:
            ...
            for result in search_results:
                    ...
                    pdf.add_image_details(result, is_pre_boxed=False) # is_pre_boxed defaults to False

...
try:
    ...
    print("Uyarı: 'torch' kütüphanesi bulunamadı. PyTorch CUDA kontrolü ve benzerlik hesaplama kullanılamayacak.")
...
argparser = argparse.ArgumentParser(description=f"{env_config.VENDOR_NAME} - {env_config.APP_NAME}") # Use constants
argparser.add_argument("--config", type=str, required=False, help="Yapılandırma dosyasının yolu.")
argparser.add_argument("--mode", type=str, choices=['development', 'production'], default='production', help="Çalıştırma modu")
argparser.add_argument("--workers", type=int, default=4, help="Gunicorn worker sayısı")
argparser.add_argument("--threads", type=int, default=2, help="Worker başına thread sayısı")
argparser.add_argument("--timeout", type=int, default=120, help="Request timeout (saniye)")
...
print("-----------------------------------")
print(f"{env_config.APP_TITLE} Başlatılıyor - {args.mode.upper()} Mode") # Use constants
print("-----------------------------------")
...
backend_info_global = "CUDA (PyTorch)" if actual_use_cuda else "CPU (Numba)" if NUMBA_AVAILABLE else "CPU (NumPy)"
...
def _setup_initial_admin_user(db_conn_config: dict, admin_username: str, admin_plain_password: str):
    """Checks for and creates the initial admin user if not present."""
    if not admin_username or not admin_plain_password:
        logger.warning("Yapılandırmada ilk admin kullanıcı adı veya parolası eksik. Admin oluşturulamadı.")
        return
    ...
    try:
        ...
        if not conn:
            logger.error("İlk admin kullanıcısı kurulumu için veritabanı bağlantısı kurulamadı.")
            return
        cursor = conn.cursor()
        ...
        hashed_password = bcrypt.hashpw(admin_plain_password.encode('utf-8'), bcrypt.gensalt()).decode('utf-8')
        ...
        conn.commit()
        logger.info(f"İlk admin kullanıcısı '{admin_username}' (bcrypt hashli) oluşturuldu.")

    except Exception as e:
        if conn:
            try: conn.rollback()
            except Exception as rb_err: logger.error(f"Rollback sırasında hata: {rb_err}")
...
def start_flask_app(config_file:str):
    ...
    # ... (config yükleme kısmı) ...
    main_conf_tuple = load_config_from_file(config_file)
    if main_conf_tuple is None:
        logger.error(f"Konfigürasyon dosyası okunamadı veya geçersiz: {config_file}")
        return
    
    # Assuming load_config_from_file returns a tuple where:
    # main_conf_tuple[0] is the main settings dictionary (that should contain 'initial_admin_user')
    # main_conf_tuple[1] is the database_config dictionary
    main_settings = main_conf_tuple[1]
    db_connection_config = main_conf_tuple[1]['database_config'] # Get db_config from the tuple
    ...
    if ONNXRUNTIME_AVAILABLE:
        ...
        if ENV_USE_CUDA and onnx_cuda_provider_available:
            logger.info("InsightFace için ONNX CUDAExecutionProvider bulundu ve ENV_USE_CUDA=True.")
            insightface_providers = ['CUDAExecutionProvider', 'CPUExecutionProvider']
        elif ENV_USE_CUDA:
            logger.warning("InsightFace için ENV_USE_CUDA=True ancak ONNX CUDAExecutionProvider bulunamadı. InsightFace CPU kullanacak.")
        else:
             logger.info("InsightFace için ENV_USE_CUDA=False. InsightFace CPU kullanacak.")
    else:
        logger.warning("onnxruntime kütüphanesi bulunamadı. InsightFace sadece CPU kullanabilir.")

    logger.info(f"InsightFace için Seçilen ONNX Runtime Sağlayıcıları: {insightface_providers}")

    # InsightFace modelini yükle (provider listesini göndererek)
    try:
        insightface_app = initilate_insightface(
             main_conf=CONFIG, # CONFIG kullanılıyor, main_conf değil
    ...
    if ALLOWED_HOSTS: # Sadece liste boş değilse kontrol et
        ...
        def check_host():
            ...
            if not (host == 'localhost' or host == '127.0.0.1' or host in ALLOWED_HOSTS):
    ...
    if admin_user_config:
        ...
        if admin_username and admin_password:
            ...
            _setup_initial_admin_user(db_connection_config, admin_username, admin_password)
        else:
            logger.warning("Yapılandırmada 'initial_admin_user' için kullanıcı adı veya parola eksik.")
    else:
        logger.warning("Yapılandırmada 'initial_admin_user' bölümü bulunamadı. Admin oluşturulamayacak.")
    ...
    logger.info(f"Kullanılacak Hesaplama Backend: {app.config.get('BACKEND_INFO', 'Bilinmiyor')}")
    ...
    if args.mode == 'production' and GUNICORN_AVAILABLE:
        ...
        logger.info(f"Gunicorn ile başlatılıyor: {args.workers} worker, {args.threads} threads")
        GunicornApplication(app, options).run()
        
    else:
        # Development mode veya Gunicorn yoksa Flask development server
        if args.mode == 'production':
            logger.warning("Production mode ama Gunicorn bulunamadı. Development server kullanılıyor!")

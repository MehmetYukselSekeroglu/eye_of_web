...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, 
                           ...
                           QHeaderView, QSplitter, QSizePolicy, QMessageBox,
                           QGraphicsDropShadowEffect, QFrame, QProgressBar, QSpacerItem)
from PyQt5.QtGui import QPixmap, QImage, QFont, QColor, QPalette, QLinearGradient, QPainter, QBrush, QPen
from PyQt5.QtCore import Qt, QTimer, pyqtSlot, QThread, pyqtSignal, QPropertyAnimation, QEasingCurve, QRect, QPoint
...
class AnimatedLabel(QLabel):
    ...
    def animate_pulse(self):
        ...
        expanded_rect = QRect(current_rect.x() - 5, current_rect.y() - 5, 
    ...
    def reset_animation(self):
        """Animasyonu sıfırla"""
        current_rect = self.geometry()
        original_rect = QRect(current_rect.x() + 5, current_rect.y() + 5, 
                            current_rect.width() - 10, current_rect.height() - 10)
        self.animation.setStartValue(current_rect)
...
class SearchThread(QThread):
    ...
    def __init__(self, face_embedding, milvus_collection, db_conn, search_threshold, max_results):
        super().__init__()
    ...
    def load_database_images(self, results):
        ...
        try:
            ...
            for hit in results[0]:
                ...
                # Get face ID
                cursor.execute('SELECT "ID" FROM "EyeOfWebFaceID" WHERE "MilvusRefID" = %s', (milvus_id,))
...
class CameraThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            self.running = False # Stop the thread if VideoCapture constructor fails critically
            return
        ...
        while self.running:
            ...
            if ret:
                ...
                if isinstance(self.camera_id, str) and self.running: # Check self.running
                    print(f"Kamera stream'i kesildi, yeniden bağlanmaya çalışılıyor: {self.camera_id}")
                    time.sleep(1)
                    if self.cap:
                        self.cap.release()
                    try:
                        self.cap = cv2.VideoCapture(self.camera_id)
                        if not self.cap or not self.cap.isOpened():
                            print(f"Yeniden bağlanma başarısız: Kamera {self.camera_id} açılamadı.")
                    ...
                    print(f"Kamera {self.camera_id} kare okuma hatası, thread durduruluyor.")
                    self.running = False 
                    break
...
class RealtimeSearchApp(QMainWindow):
    ...
    def __init__(self):
        ...
        # App configuration
        self.setWindowTitle("👁️ EyeOfWeb - Kurumsal Yüz Tanıma Sistemi v2.0")
        ...
        self.face_search_threads = {}  # Store search threads for each face {face_index: thread}
        self.face_results = {}  # Store results for each face {face_index: results}
        self.face_db_images = {}  # Store DB images for each face {face_index: {milvus_id: image}}
    ...
    def load_config_and_analyzer(self):
        ...
        try:
            ...
            if not config_data[0]:
                QMessageBox.critical(self, "Hata", f"Konfigürasyon yüklenemedi: {config_data[1]}")
                sys.exit(1)
            ...
            QMessageBox.critical(self, "Hata", f"Konfigürasyon ve model yükleme hatası: {str(e)}")
            sys.exit(1)
            
    def connect_to_milvus(self):
        """Milvus veritabanına bağlan"""
        try:
            # Connect to Milvus
            connections.connect(
                alias="default",
            ...
            self.milvus_collection = get_milvus_collection(collection_name, alias="default")
            
            if self.milvus_collection:
                print(f"Milvus koleksiyonuna başarıyla bağlanıldı: {collection_name}")
            else:
                QMessageBox.warning(self, "Uyarı", f"Milvus koleksiyonu '{collection_name}' yüklenemedi.")
                
        except Exception as e:
            QMessageBox.critical(self, "Milvus Hatası", f"Milvus'a bağlanılamadı: {str(e)}")
    ...
    def connect_to_postgres(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "Veritabanı Hatası", f"PostgreSQL'e bağlanılamadı: {str(e)}")
    ...
    def create_camera_section(self):
        ...
        self.manual_camera_input.setPlaceholderText("rtsp://ip:port/stream, http://ip/video.mjpg, /dev/video0, 0")
    ...
    def create_results_section(self):
        ...
        self.results_table.setHorizontalHeaderLabels(["🆔 ID", "🎯 Benzerlik", "👤 Cinsiyet", "🎂 Yaş"])
    ...
    def create_faces_grid(self):
        ...
        print(f"Creating face grid with {grid_rows}x{grid_cols} = {grid_rows * grid_cols} face widgets")
    ...
    def create_face_widget(self, face_index):
        ...
        print(f"Created face widget {face_index} with size {face_widget.size()}, image size {face_image.size()}")
            ...
            if hasattr(widget, 'face_index') and not sip.isdeleted(widget):
                try:
                    if widget.face_index == face_index:
                        ...
                        if hasattr(widget, 'face_image') and not sip.isdeleted(widget.face_image):
                            ...
                            """)
                        if hasattr(widget, 'face_title') and not sip.isdeleted(widget.face_title):
    ...
    def update_selected_face_display(self):
        ...
        if not hasattr(self, 'selected_face_view') or not hasattr(self, 'selected_db_view'):
        ...
        try:
            if (face_index < len(self.detected_faces) and 
                face_index < len(self.face_widgets) and 
                hasattr(self.face_widgets[face_index], 'face_image') and
                not sip.isdeleted(self.face_widgets[face_index].face_image)):
                ...
                if pixmap and not sip.isdeleted(self.selected_face_view):
                    scaled_pixmap = pixmap.scaled(self.selected_face_view.size(), Qt.KeepAspectRatio, Qt.SmoothTransformation)
        ...
        try:
            ...
            has_db_images = (face_index in self.face_db_images and 
                           self.face_db_images[face_index])
            has_search_results = (face_index in self.face_results and 
                                self.face_results[face_index] and 
                                self.face_results[face_index][0])
            
            if has_db_images and has_search_results and not sip.isdeleted(self.selected_db_view):
                ...
                if pixmap:
                    scaled_pixmap = pixmap.scaled(self.selected_db_view.size(), Qt.KeepAspectRatio, Qt.SmoothTransformation)
                ...
                if has_search_results:
                    self.selected_db_view.setText("🗄️ VERİTABANI GÖRÜNTÜSÜ YÜKLENİYOR...")
                    print(f"No database images loaded yet for face {face_index}")
                else:
                    self.selected_db_view.setText("❌ EŞLEŞME BULUNAMADI")
                    print(f"No search results for face {face_index}, clearing database view")
        except RuntimeError as e:
            if "wrapped C/C++ object" in str(e):
    ...
    def create_footer(self):
        ...
        # Ana durum
        self.status_label = AnimatedLabel("🟢 SİSTEM HAZIR")
        self.status_label.setObjectName("mainStatus")
        
        # İstatistikler
        self.stats_label = QLabel("📈 İstatistikler: 0 arama, 0 eşleşme")
    ...
    def apply_cybernetic_theme(self):
        ...
        QPushButton#cyberButtonActive:hover {
            background: qlineargradient(x1:0, y1:0, x2:1, y2:1, 
                stop:0 #0e5c0e, stop:1 #0c4a0c);
            border: 1px solid #0e5c0e;
        }
        ...
        QComboBox#cyberCombo::down-arrow {
            image: none;
        ...
        QComboBox#cyberCombo QAbstractItemView {
        ...
        QLineEdit#cyberLineEdit::placeholder {
            color: #888888;
            font-style: italic;
        }
        ...
        QSlider#cyberSlider::handle:horizontal:hover {
            background: qlineargradient(x1:0, y1:0, x2:1, y2:1, 
        ...
        QTableWidget#cyberTable::item:hover {
        ...
        #controlLabel, #paramLabel, #tabTitle {
            color: #ffffff;
            font-size: 11pt;
            font-weight: 600;
        }
        ...
        QProgressBar#cyberProgress::chunk {
            background: qlineargradient(x1:0, y1:0, x2:1, y2:0, 
        ...
        try:
            ...
            if self.detected_faces:
                ...
                for i, face in enumerate(self.detected_faces):
                    if i >= self.max_faces:  # Limit to max faces
                        print(f"Skipping face {i}, exceeds max faces ({self.max_faces})")
                        break
                        
                    bbox = face.bbox.astype(int)
                    print(f"Processing face {i} with bbox: {bbox}")
                    
                    # Draw face box with different colors for each face
                    colors = [(212, 120, 0), (0, 255, 0), (255, 0, 0), (255, 255, 0), (255, 0, 255), (0, 255, 255)]
                    color = colors[i % len(colors)]
                    cv2.rectangle(frame, (bbox[0], bbox[1]), (bbox[2], bbox[3]), color, 2)
                    
                    # Yüz ID'si ekle
                    cv2.putText(frame, f"YUZ-{i+1}", (bbox[0], bbox[1]-10), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
                    
                    # Update face widget with error handling
                    print(f"Updating face widget {i}")
                    try:
                        self.update_face_widget(i, face, frame)
                    except RuntimeError as e:
                        if "wrapped C/C++ object" in str(e):
                            print(f"Face widget {i} deleted during update, skipping")
                            continue
                        else:
                            raise
                
                # Clear unused face widgets
                for i in range(len(self.detected_faces), len(self.face_widgets)):
                    if i < len(self.face_widgets):
                        print(f"Clearing unused face widget {i}")
                        try:
                ...
                try:
                    ...
                    if "wrapped C/C++ object" in str(e):
                        print("Camera view deleted, skipping frame update")
                        return
                    else:
                        raise
                
                # Auto search if enabled and interval passed
                if (self.auto_search_toggle.isChecked() and 
                    time.time() - self.last_search_time > self.search_interval and
                    self.detected_faces and not self.is_searching):
    ...
    def update_face_widget(self, face_index, face, frame):
        ...
        if face_index >= len(self.face_widgets):
            print(f"Face index {face_index} out of range, max widgets: {len(self.face_widgets)}")
            return
            
        face_widget = self.face_widgets[face_index]

        if sip.isdeleted(face_widget) or \
           not hasattr(face_widget, 'face_image') or sip.isdeleted(face_widget.face_image) or \
           not hasattr(face_widget, 'status_label') or sip.isdeleted(face_widget.status_label):
            print(f"Skipping update for deleted face_widget or its QLabel children at index {face_index}")
            return
        ...
        try:
            ...
            if x2 <= x1 or y2 <= y1:
                print(f"Invalid face region for face {face_index}: ({x1},{y1}) to ({x2},{y2})")
                return
            ...
            qt_image = QImage(face_img_rgb.data, img_w, img_h, bytes_per_line, QImage.Format_RGB888)
            ...
            try:
                ...
                if face_widget.face_image.pixmap():
                    print(f"✅ Successfully updated face widget {face_index} with image {scaled_pixmap.size().width()}x{scaled_pixmap.size().height()}")
                else:
            ...
            try:
                ...
                if "wrapped C/C++ object" in str(e):
                    print(f"Face status label {face_index} deleted during status update")
                    return
                else:
                    raise
    ...
    def search_all_faces(self):
        ...
        for i, face in enumerate(self.detected_faces):
            ...
            if i not in self.face_search_threads or not self.face_search_threads[i].isRunning():
    ...
    def search_face(self, face_index, face):
        ...
        try:
            ...
            if face_index < len(self.face_widgets):
                face_widget = self.face_widgets[face_index]
                if not sip.isdeleted(face_widget) and \
                   hasattr(face_widget, 'status_label') and not sip.isdeleted(face_widget.status_label):
                    try:
                        ...
                        if "wrapped C/C++ object" in str(e):
                            print(f"Face status label {face_index} deleted during search status update")
                        else:
                            raise
            ...
            search_thread.search_completed.connect(lambda results, idx=face_index: self.on_face_search_completed(idx, results))
            search_thread.search_error.connect(lambda error, idx=face_index: self.on_face_search_error(idx, error))
            search_thread.images_loaded.connect(lambda images, idx=face_index: self.on_face_images_loaded(idx, images))
    ...
    def on_face_search_completed(self, face_index, results):
        ...
        self.stats_label.setText(f"📈 İstatistikler: {self.total_searches} arama, {self.successful_matches} eşleşme")
    ...
    def get_db_image_pixmap_for_face(self, face_index, milvus_id):
        ...
        if (face_index not in self.face_db_images or 
            milvus_id not in self.face_db_images[face_index]):
            return None
    ...
    def update_results_table(self, results):
        ...
        try:
            self.results_indicator.setText(f"📊 {len(filtered_results)} sonuç bulundu (Eşik: {threshold:.2f})")
        except RuntimeError as e:
            if "wrapped C/C++ object" in str(e):
                print("Results indicator deleted, skipping update")
    ...
    def on_result_selected(self, item):
        ...
        try:
            ...
            if pixmap and not sip.isdeleted(self.selected_db_view):
                ...
                try:
                    self.selected_db_view.clear()
                    self.selected_db_view.setText("❌ VERİTABANI GÖRÜNTÜSÜ MEVCUT DEĞİL")
                except RuntimeError as e:
                    if "wrapped C/C++ object" in str(e):
            ...
            if "wrapped C/C++ object" in str(e):
                print("Results table or selected DB view deleted during result selection")
            else:
    ...
    def resizeEvent(self, event):
        ...
        if self.current_frame is not None:
            ...
            bytes_per_line = c * w
            qt_image = QImage(self.current_frame.data, w, h, bytes_per_line, QImage.Format_RGB888).rgbSwapped()
            pixmap = QPixmap.fromImage(qt_image)
            scaled_pixmap = pixmap.scaled(self.camera_view.size(), Qt.KeepAspectRatio, Qt.SmoothTransformation)
            self.camera_view.setPixmap(scaled_pixmap)

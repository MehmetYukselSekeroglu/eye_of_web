...
def get_or_create_collection(
    collection_name, fields, primary_field_name, description=""
):
    """
    ...
    try:
        if not connections.has_connection(MILVUS_ALIAS):
            print(
                f"Connecting to Milvus at {MILVUS_HOST}:{MILVUS_PORT} under alias '{MILVUS_ALIAS}'..."
            )
    ...
    if utility.has_collection(collection_name, using=MILVUS_ALIAS):
        ...
        collection = Collection(
            collection_name, schema=schema, using=MILVUS_ALIAS, **COLLECTION_PARAMS
        )
...
custom_face_milvus_fields = [
    ...
    FieldSchema(
        name="detection_date_ts",
        dtype=DataType.INT64,
        description="Detection timestamp (Unix epoch)",
    ),
    ...
    for config in collection_configs:
        ...
        for vec_idx_config in config.get("vector_indexes", []):
            ...
            if not collection.has_index(index_name=vector_index_name):
                print(
                    f"Creating vector index '{vector_index_name}' on field '{vector_field_name}' for collection '{config['name']}' with params {index_params}..."
                )
                collection.create_index(
                    vector_field_name, index_params, index_name=vector_index_name
                )
                print(f"Vector index '{vector_index_name}' created.")
            else:
                print(
                    f"Vector index '{vector_index_name}' already exists on collection '{config['name']}'."
                )
        ...
        for scalar_field in config.get("scalar_indexes", []):
            ...
            if not collection.has_index(index_name=scalar_index_name):
                try:
                    print(
                        f"Creating scalar index '{scalar_index_name}' on field '{scalar_field}' for collection '{config['name']}'..."
                    )
                    ...
                    print(
                        f"Could not create scalar index '{scalar_index_name}' on field '{scalar_field}': {e}"
                    )
            else:
                print(
                    f"Scalar index '{scalar_index_name}' already exists on collection '{config['name']}'."
                )
        ...
        if indexed_vector_fields:
            print(
                f"Loading collection '{config['name']}' for indexed vector fields: {indexed_vector_fields}..."
            )
            collection.load(field_names=indexed_vector_fields)
            print(
                f"Collection '{config['name']}' loaded for fields: {indexed_vector_fields}."
            )
        else:
            ...
            print(
                f"Loading collection '{config['name']}' with a generic load call (no specific indexed vector fields found in this configuration)."
            )
            collection.load()

...
from flask import Blueprint, render_template, request, redirect, url_for, flash, session, abort, g, jsonify, send_file
...
import ast
from pymilvus import Collection # Added for type hinting if g.db_tools uses it, but direct use is avoided
...
EYE_OF_WEB_FACE_DATA_MILVUS_COLLECTION_NAME = "EyeOfWebFaceDataMilvus" # Defined in MILVUS_SCHEMA_GENERATOR.py
...
def validate_and_sanitize_image(file_storage):
    ...
    if not allowed_file(filename):
        return None, f"İzin verilmeyen dosya türü. İzin verilenler: {', '.join(ALLOWED_EXTENSIONS)}"
    ...
    try:
        ...
        output_format = img.format if img.format in ['JPEG', 'PNG', 'GIF', 'WEBP'] else 'PNG'
        ...
        if img.mode == 'P': # Paletli ise
            ...
            img = img.convert("RGB")
        elif img.mode != 'RGB' and img.mode != 'L' and img.mode != 'RGBA': # L = grayscale
        ...
        current_app.logger.error(f"Resim işleme sırasında beklenmedik hata ({filename}): {e}\n{traceback.format_exc()}")
...
def set_jwt_cookie(response):
    ...
    if hasattr(g, 'jwt_token_for_cookie') and g.jwt_token_for_cookie:
        ...
        try:
            ...
            response.set_cookie(
                key=current_app.config.get('JWT_ACCESS_COOKIE_NAME', 'access_token_cookie'),
                value=token,
                httponly=True,
            ...
            current_app.logger.error(f"JWT cookie ayarlanamadı: {e}. Response tipi: {type(response)}")
...
def search():
    ...
    search_performed = any(search_params.values()) # Check if any search param is present in GET
    ...
    if search_performed:
        ...
        try:
            ...
            # ---> ADD LOGGING HERE <---
            print(f"[ROUTE /search GET] Controller Result: success={success}, msg='{message}', len(results)={len(results)}, total={total}")
...
def search_by_image():
    ...
    if request.method == 'POST':
        ...
        print(f"[ROUTE /search/image POST] Algorithm: {algorithm}, Use CUDA: {use_cuda_backend}")
        ...
        try:
            ...
            img_array = np.array(img.convert('RGB')) # RGB'ye dönüştürme zaten validate_and_sanitize_image içinde yapılıyor olabilir, ama burada tekrar etmek sorun olmaz.
            ...
            if search_results_raw:
                ...
                for result_item in search_results_raw:
                    ...
                    if not face_id:
                        current_app.logger.warning(f"Skipping result with missing face ID: {result_item}")
                        continue
                    ...
                    try:
                        ...
                        if db_image_details:
                            ...
                            if binary_image_data:
                                try:
                                    decompressed_bytes = decompress_image(binary_image_data)
                                    if decompressed_bytes:
                                        image_base64 = base64.b64encode(decompressed_bytes).decode('utf-8')
                                        # Basic MIME type detection
                                        if decompressed_bytes.startswith(b'\x89PNG\r\n\x1a\n'):
                                            image_mime_type = 'image/png'
                                        elif decompressed_bytes.startswith(b'\xff\xd8\xff'):
                                            image_mime_type = 'image/jpeg'
                                        elif decompressed_bytes.startswith(b'RIFF') and decompressed_bytes[8:12] == b'WEBP':
                                            image_mime_type = 'image/webp'
                                        else:
                                            image_mime_type = 'application/octet-stream' # Fallback MIME type
                                        current_app.logger.debug(f"Successfully generated Base64 for face {face_id} after decompression. MIME: {image_mime_type}")
                                    else:
                                        current_app.logger.warning(f"Decompression failed or returned empty for face {face_id}")
                                except Exception as decomp_encode_err:
                                    current_app.logger.error(f"Error decompressing/encoding DB image for face {face_id}: {decomp_encode_err}")
                                    image_base64 = None # Ensure it's None on error
                            
                            if image_base64 is None: 
                                image_url = build_image_url(
                                    db_image_details.get('ImageProtocol'), # Use get with default
                                    db_image_details.get('image_domain'),
                                    db_image_details.get('image_path'),
                                    db_image_details.get('image_etc')
                                )
                                if image_url:
                                     current_app.logger.debug(f"Using URL for face {face_id} as Base64 is unavailable or failed.")
                                else:
                                     current_app.logger.warning(f"Could not generate Base64 or build URL for face {face_id}")
                        else:
                            current_app.logger.warning(f"Could not find image details in DB for face ID: {face_id}")
                            
                    except Exception as detail_err:
                        current_app.logger.error(f"Error fetching details for face ID {face_id}: {detail_err}")
            ...
            flash(f'Görsel işlenirken veya sonuçlar hazırlanırken hata oluştu: {str(e)}', 'danger')
            current_app.logger.error(f"Error in search_by_image POST: {traceback.format_exc()}")
            return redirect(request.url)
...
def download_image_search_report():
    ...
    if not search_results:
        flash('İndirilecek rapor verisi bulunamadı veya oturum süresi doldu.', 'warning')
    ...
    try:
        ...
        for result in search_results:
            ...
            if not db_pg_data:
                print(f"Uyarı: Veritabanında face_id veya ilişkili kayıt bulunamadı: {res_face_id}")
            ...
            try:
                ...
                if not milvus_attributes:
                    current_app.logger.warning(f"Milvus'tan {res_face_id} için öznitelik alınamadı.")
            except Exception as e:
                current_app.logger.error(f"Milvus'tan {res_face_id} için öznitelik alınırken hata: {e}")
            
            # --- PDF Öğesini Oluştur --- 
            title = f"Yüz ID: {res_face_id}" 
            
            # Cinsiyet formatını düzelt (Milvus'tan)
            gender_bool = milvus_attributes.get('face_gender') if milvus_attributes else None
            gender_str = 'Erkek' if gender_bool is True else ('Kadın' if gender_bool is False else 'N/A')
            
            # Yaş, Skor, Facebox (Milvus'tan)
            age_from_milvus = milvus_attributes.get('face_age') if milvus_attributes else None
            score_from_milvus = milvus_attributes.get('detection_score') if milvus_attributes else None
            facebox_from_milvus = milvus_attributes.get('face_box') if milvus_attributes else None
            ...
            if binary_image_data:
                try:
                    ...
                    if decompressed_image_bytes:
                        image_data_b64 = base64.b64encode(decompressed_image_bytes).decode('utf-8')
                    else:
                        current_app.logger.warning(f"PDF Raporu (image_search): {res_face_id} için dekompresyon başarısız veya boş veri.")
                except Exception as e:
                    current_app.logger.error(f"PDF Raporu (image_search): {res_face_id} için dekompresyon/encode hatası: {e}")
            ...
            pdf_item = {
                ...
                'gender': gender_str, 
                'age': age_from_milvus,
                'similarity': {'name': f'Yüklenen Görsel ({image_name})', 'rate': result.get('similarity', 0)}, 
        ...
        if conn: g.db_tools.releaseConnection(conn, cursor)
    ...
    report_filename = f"EyeOfWeb_GorselArama_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    return send_file(
...
def search_similar(face_id):
    ...
    use_cuda_backend = current_app.config.get('USE_CUDA', False)
    print(f"[ROUTE /search/similar GET] FaceID: {face_id}, Algorithm: {algorithm}, Threshold: {threshold}, Use CUDA: {use_cuda_backend}")
    ...
    try:
        ...
        if target_embedding is None:
            flash(f'Hedef yüz (ID: {target_face_id}) için embedding verisi bulunamadı veya alınamadı.', 'danger')
            return redirect(url_for('web.index'))
            ...
            for face in similar_faces:
                ...
                if image_id:
                    ...
                    if success_img and img_binary:
                        try:
                            ...
                            face['image_data'] = base64.b64encode(img_binary).decode('utf-8')
                            if img_binary.startswith(b'\x89PNG\r\n\x1a\n'): face['image_mime_type'] = 'image/png'
                            elif img_binary.startswith(b'\xff\xd8\xff'): face['image_mime_type'] = 'image/jpeg'
                            else: face['image_mime_type'] = 'image/jpeg'
                        except Exception as encode_err:
                            current_app.logger.error(f"Similar face image (ID: {image_id}) encode error: {encode_err}")
                            face['image_data'] = None
                    # else:
                ...
                if face['image_data'] is None and image_info:
                    ...
                    if img_domain and img_path:
                        ...
                        current_app.logger.debug(f"Constructed URL for similar face {face.get('id')}: {url}")
                    else:
                         current_app.logger.warning(f"Could not construct URL for similar face {face.get('id')} due to missing components in image_info: {image_info}")

                # Eğer image_id yoksa ve URL de oluşturulamadıysa logla
                if not image_id and face['original_image_url'] is None:
                     current_app.logger.warning(f"Similar face (ID: {face.get('id')}) is missing image_id and valid image_info for URL.")
        ...
        return render_template('face_similarity.html',
        ...
        current_app.logger.error(f"Benzer yüz arama hatası (Face ID: {face_id}): {e}\n{traceback.format_exc()}")
        flash(f'Benzer yüzler aranırken beklenmedik bir hata oluştu: {str(e)}', 'danger')
...
def download_similar_search_report():
    ...
    search_type = f"Benzer Yüz Araması (Kaynak: {original_face_desc}, Eşik: {threshold})"
    ...
    try:
        ...
        def check_source(face_id_to_check):
            # WhiteList kontrolü
            cursor.execute('SELECT 1 FROM "WhiteListFaces" WHERE "ID" = %s LIMIT 1', (face_id_to_check,))
            if cursor.fetchone():
                return " [Beyaz Liste]"
            # EGM kontrolü
            cursor.execute('SELECT 1 FROM "EgmArananlar" WHERE "ID" = %s LIMIT 1', (face_id_to_check,))
            if cursor.fetchone():
                return " [EGM]"
            return ""
        ...
        for result in search_results:
            ...
            pdf_item = {
                ...
                'gender': gender_str, 
                'age': age_from_milvus,
                'similarity': {'name': f'Kaynak Yüz ({original_face_desc})', 'rate': result.get('similarity', 0)}, 
                'comprehensive_info': None,
                'facebox': facebox_from_milvus 
            }
            pdf_data.append(pdf_item)
        ...
        # Hata durumunda bile bağlantıyı kapat
        if conn: g.db_tools.releaseConnection(conn, cursor)
        return redirect(url_for('web.face_details', face_id=face_id)) # Benzer arama için face_details'a yönlendir
    finally:
        # Bağlantıyı kapat
        if conn: g.db_tools.releaseConnection(conn, cursor)
...
def dashboard():
    ...
    try:
        ...
        # Toplam yüz sayısı
        result = g.db_tools.executeQuery("SELECT COUNT(*) as count FROM \"ImageBasedMain\"")
        if result and len(result) > 0:
            stats['total_faces'] = result[0]['count']
        
        # Toplam domain sayısı
        result = g.db_tools.executeQuery("SELECT COUNT(DISTINCT \"BaseDomainID\") as count FROM \"ImageBasedMain\"")
        if result and len(result) > 0:
            stats['total_domains'] = result[0]['count']
        
        # Toplam görsel sayısı
        result = g.db_tools.executeQuery("SELECT COUNT(DISTINCT \"ImageID\") as count FROM \"ImageBasedMain\"")
        if result and len(result) > 0:
            stats['total_images'] = result[0]['count']
        
        # Yüksek riskli yüz sayısı
        result = g.db_tools.executeQuery("SELECT COUNT(*) as count FROM \"ImageBasedMain\" WHERE \"RiskLevel\" = 'yüksek'")
        if result and len(result) > 0:
        ...
        if table_query and len(table_query) > 0:
            for table in table_query:
                ...
                count_query = g.db_tools.executeQuery(f"SELECT COUNT(*) as count FROM \"{table_name}\"")
        ...
        if recent_scans and len(recent_scans) > 0:
            for scan in recent_scans:
                ...
                stats['recent_scans'].append({
                    ...
                    'date': scan['DetectionDate'].strftime('%d.%m.%Y %H:%M') if scan['DetectionDate'] else '',
...
def whitelist_search():
    ...
    if request.method == 'POST':
        ...
        if request.form.get('institution'):
            search_params['institution'] = html.escape(request.form.get('institution', ''))
        ...
        if start_date:
            try:
                ...
                flash('Başlangıç tarihi formatı geçersiz. YYYY-AA-GG formatında olmalıdır.', 'warning')
        ...
        if end_date:
            try:
                ...
                flash('Bitiş tarihi formatı geçersiz. YYYY-AA-GG formatında olmalıdır.', 'warning')
                return redirect(url_for('web.whitelist_search'))
        ...
        success, message, results = SearchController.search_whitelist_faces(search_params)
        ...
        return render_template('whitelist_results.html', results=results, params=search_params)
...
def whitelist_search_by_face(face_id):
    ...
    try:
        ...
        # Benzer yüzleri ara
        success, message, results = SearchController.search_by_embedding(face_embedding, threshold)
        ...
        return render_template('whitelist_similar_results.html', 
                              results=results,
...
def external_search():
    ...
    if request.method == 'POST':
        ...
        # En az bir arama parametresi gerekli
        valid_params = {k: v for k, v in search_params.items() if k != 'alarm' or v is not None}
        if not valid_params:
            flash('En az bir arama kriteri belirtmelisiniz', 'warning')
            return redirect(url_for('web.external_search'))
        
        # Arama işlemini gerçekleştir
        success, message, results = SearchController.search_external_faces(search_params)
        ...
        return render_template('external_results.html', results=results, params=search_params)
...
def egm_search():
    ...
    try:
        ...
        if request.method == 'GET':
            return render_template('egm_search.html', 
        ...
        return render_template('egm_results.html', results=results, params=search_params)
...
def egm_search_by_face(face_id):
    try:
        ...
        if original_face.get('embedding'):
            face_embedding = np.array(ast.literal_eval(original_face['embedding']), dtype=np.float32)
            ...
            filtered_results = [face for face in similar_faces if str(face.get('id', '')) != str(face_id)]
            
            return render_template('egm_similar_results.html', 
                                  original_face=original_face,
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('search_results.html', 
                                   ...
                                   message=message,
                                   query_type="görsel",
                                   image_name=secure_name) # YENİ: Güvenli dosya adı eklendi
        except Exception as e:
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if not whitelist_results:
                flash('Yüklediğiniz görselde beyaz listeyle eşleşen yüzler bulunamadı.', 'info')
                return redirect(request.url)
            
            # Sonuçları göster
            return render_template('whitelist_results.html', 
                                   results=whitelist_results, 
                                   message=f"{len(whitelist_results)} adet beyaz liste eşleşmesi bulundu.",
                                   params={'threshold': threshold, 'query_type': 'görsel', 'image_name': secure_name},
...
def egm_upload_search():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if not egm_results:
                flash('Yüklediğiniz görselde EGM arananlar listesiyle eşleşen yüzler bulunamadı.', 'info')
                return redirect(request.url)
            
            # Sonuçları göster
            return render_template('egm_results.html', 
                                   results=egm_results, 
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if results and results.get('faces'):
                ...
                for idx, face in enumerate(results['faces']):
                    ...
                    if bbox and len(bbox) == 4:
                        ...
                        # Add index number
                        text = f"#{idx+1}"
                        font = cv2.FONT_HERSHEY_SIMPLEX; font_scale = 0.6; text_color = (255, 255, 255); text_thickness = 1
                        (text_width, text_height), _ = cv2.getTextSize(text, font, font_scale, text_thickness)
                        text_x = x1; text_y = y1 - 5 if y1 - 5 > text_height else y1 + text_height + 5
                        cv2.rectangle(img_with_boxes, (text_x, text_y - text_height - 2), (text_x + text_width + 2, text_y + 2), color, -1)
                        cv2.putText(img_with_boxes, text, (text_x + 1, text_y - 1), font, font_scale, text_color, text_thickness, cv2.LINE_AA)
                ...
                for idx, face in enumerate(results['faces']):
                    gender_str = 'Erkek' if face.get('gender') == 1 else ('Kadın' if face.get('gender') == 0 else 'N/A')
                    pdf_item = {
            ...
            return render_template('face_detection.html', 
                                   results=results,
                                   message=message,
                                   report_available=report_available) # Pass the flag
        except Exception as e:
            flash(f'Görsel işlenirken hata oluştu: {str(e)}', 'danger')
            current_app.logger.error(f"Yüz tespiti hatası: {str(e)}\n{traceback.format_exc()}")
            return redirect(request.url)
...
def face_comparison():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Yüz karşılaştırma
            success, message, results = SearchController.compare_faces(img1_bgr, img2_bgr, threshold)
            ...
            if results:
                ...
                if results.get('face1') and results['face1'].get('bbox'):
                    ...
                    if fy2 > fy1 and fx2 > fx1:
                        ...
                        face1_cropped_b64 = base64.b64encode(buffer_crop1).decode('utf-8')
                ...
                if results.get('face2') and results['face2'].get('bbox'):
                    ...
                    if fy2 > fy1 and fx2 > fx1:
                        ...
                        face2_cropped_b64 = base64.b64encode(buffer_crop2).decode('utf-8')
                    # Draw box on the other copy
                ...
                pdf_item1 = {
                    ...
                    'gender': 'Erkek' if results.get('face1', {}).get('gender') == 1 else ('Kadın' if results.get('face1', {}).get('gender') == 0 else 'N/A'),
                ...
                pdf_item2 = {
                    ...
                    'gender': 'Erkek' if results.get('face2', {}).get('gender') == 1 else ('Kadın' if results.get('face2', {}).get('gender') == 0 else 'N/A'),
            ...
            flash(f'Görsel işlenirken hata oluştu: {str(e)}', 'danger')
            current_app.logger.error(f"Yüz karşılaştırma hatası: {str(e)}\n{traceback.format_exc()}")
            return redirect(request.url)
...
def text_search():
    ...
    if request.method == 'POST':
        ...
        return render_template('text_search_results.html', 
                               results=results, 
...
def fetch_domain_faces():
    ...
    if not face_id:
        return jsonify({
            'success': False,
            'message': 'Face ID parametresi gerekli',
            'faces': []
        })
    ...
    try:
        ...
        image_url = build_image_url(image_protocol, image_domain, image_path, image_url_etc)
        ...
        # Facebox verisi
        facebox_data = np.array(ast.literal_eval(facebox), dtype=np.float32).tolist() # JSON için listeye çevir
...
def deep_insight(face_id):
    ...
    try:
        ...
        if not image_results:
            ...
            return render_template('deep_insight.html', 
                                  face=face_details,
                                  relationships=[],
                                  stats={})
            ...
            for other_face_id in faces_in_image:
                if other_face_id != face_id:
                    ...
                    if other_face_id not in face_co_occurrence_details:
                        ...
                        if face_result:
                            # URL oluştur
                            image_url = build_image_url(face_result['protocol'], face_result['domain'], face_result['path'], face_result['url_etc'])
                            ...
                            face_co_occurrence_details[other_face_id] = {
                                ...
                                'facebox': np.array(ast.literal_eval(face_result['facebox']), dtype=np.float32).tolist() if face_result['facebox'] else None
                            }
...
def face_similarity_analysis(face_id):
    ...
    try:
        ...
        min_cooccurrence = int(request.args.get('min_cooccurrence', 2))  # Min. birlikte görünme sayısı

        current_app.logger.debug(f"Benzerlik analizi başlıyor: Eşik={similarity_threshold}, Min. Birliktelik={min_cooccurrence}")
        ...
        success, message, target_face_details = SearchController.get_face_details(target_face_id)
        if not success:
            current_app.logger.warning(f"Hedef yüz detayları alınamadı (ID: {target_face_id}): {message}")
        ...
        if not image_results:
            g.db_tools.releaseConnection(conn, cursor)
            flash(f'Hedef yüzün (ID: {target_face_id}) bulunduğu görsel bulunamadı.', 'warning')
            return render_template('face_similarity_pairs.html',
                                  target_face=target_face_details,
                                  related_pairs=[],
                                  stats={'threshold': similarity_threshold, 'min_cooccurrence': min_cooccurrence})
        ...
        current_app.logger.info(f"Analiz edilecek toplam {len(all_face_ids_in_related_images)} ilişkili yüz bulundu.")
        ...
        if all_face_ids_in_related_images:
            ...
            for face_data in face_data_batch:
                ...
                if face_data['FaceEmbeddingData']:
                    try:
                        embedding = np.array(ast.literal_eval(face_data['FaceEmbeddingData']), dtype=np.float32)
                        if embedding.size == 512:
                            face_embeddings[face_id] = embedding
                            face_details_cache[face_id] = {
                                'id': face_id,
                                'gender': 'Erkek' if face_data['FaceGender'] else 'Kadın',
                                'age': face_data['FaceAge']
                            }
                        else:
                             current_app.logger.warning(f"Yüz ID {face_id} için beklenmeyen embedding boyutu: {embedding.size}")
                    except Exception as emb_err:
                        current_app.logger.error(f"Yüz ID {face_id} için embedding parse hatası: {emb_err}")
                else:
                     current_app.logger.warning(f"Yüz ID {face_id} için embedding verisi bulunamadı.")
        ...
        current_app.logger.info(f"Hesaplanacak toplam ilişkili yüz çifti: {total_combinations}")
        ...
        for face_id1, face_id2 in itertools.combinations(valid_face_ids, 2):
            ...
            if processed_combinations % 1000 == 0:
                 current_app.logger.debug(f"{processed_combinations}/{total_combinations} çift işlendi.")

            if similarity >= similarity_threshold:
                ordered_pair = tuple(sorted((face_id1, face_id2)))
                face_pairs_above_threshold[ordered_pair] = float(similarity)

        current_app.logger.info(f"Eşik ({similarity_threshold}) üzerinde {len(face_pairs_above_threshold)} ilişkili yüz çifti bulundu.")
        ...
        if face_ids_to_fetch_details_for:
            ...
            for row in detail_results:
                ...
                original_url = build_image_url(
                    row['ImageProtocol'], row['image_domain'], row['image_path'], row['image_etc']
                )
                facebox = np.array(ast.literal_eval(row['FaceBox']), dtype=np.float32).tolist() if row['FaceBox'] else None
        ...
        # 7. Sonuçları Sırala
        final_related_pairs.sort(key=lambda x: (x['co_occurrence_count'], x['similarity']), reverse=True)
        ...
        g.db_tools.releaseConnection(conn, cursor)
        current_app.logger.info(f"Yüz {target_face_id} için detaylı ilişki analizi tamamlandı. {len(final_related_pairs)} ilişkili çift bulundu.")
        ...
        if conn: g.db_tools.releaseConnection(conn, cursor if 'cursor' in locals() else None)
        current_app.logger.error(f"Yüz benzerlik analizi (ID: {face_id}) sırasında hata: {traceback.format_exc()}")
...
def extended_face_analysis(face_id):
    ...
    flash('Bu analiz aracı güncellenmektedir. Lütfen Kapsamlı Kişi Analizi\\\'ni kullanın.', 'info')
...
def comprehensive_person_analysis(face_id):
    ...
    try:
        ...
        if not target_face_pg_info:
            flash(f'Hedef yüz (ID: {target_face_id}) PostgreSQL\'de bulunamadı.', 'error')
            if conn: g.db_tools.releaseConnection(conn, cur)
            return redirect(url_for('web.dashboard'))
        ...
        try:
            ...
            if not target_milvus_data or 'face_embedding_data' not in target_milvus_data:
                flash(f'Hedef yüz (ID: {target_face_id}) için Milvus\'ta embedding verisi bulunamadı.', 'error')
                if conn: g.db_tools.releaseConnection(conn, cur)
                return redirect(url_for('web.dashboard'))
            ...
            if target_embedding.size != 512:
                 raise ValueError(f"Milvus'tan gelen beklenmeyen embedding boyutu: {target_embedding.size}")

        except AttributeError as attr_err:
            current_app.logger.error(f"Milvus veri getirme fonksiyonu g.db_tools'da bulunamadı: {attr_err}")
            flash('Milvus altyapısında bir sorun var (get_milvus_face_attributes). Lütfen yönetici ile iletişime geçin.', 'error')
            if conn: g.db_tools.releaseConnection(conn, cur)
            return redirect(url_for('web.dashboard'))
        except Exception as milvus_fetch_err:
             current_app.logger.error(f"Hedef yüz ({target_face_id}) Milvus veri getirme hatası: {milvus_fetch_err}\\{traceback.format_exc()}")
             flash(f'Hedef yüz için Milvus\'tan veri alınırken hata: {str(milvus_fetch_err)}', 'error')
             if conn: g.db_tools.releaseConnection(conn, cur)
             return redirect(url_for('web.dashboard'))
            ...
            if success_img and img_binary:
                try:
                    ...
                    if decompressed_binary:
                        target_face_image_data = base64.b64encode(decompressed_binary).decode('utf-8')
                        target_face_mime_type = 'image/png' 
                    else:
                         current_app.logger.warning(f"Hedef yüz ({target_face_id}) için dekompresyon boş veri döndürdü.")
                except Exception as encode_err:
                    current_app.logger.error(f"Hedef yüz ({target_face_id}) resim dekompres/encode hatası: {encode_err}")

        # 3. Hedef yüze benzer yüzleri bul (Milvus kullanarak)
        current_app.logger.info(f"Hedef yüze ({target_face_id}) benzer yüzler Milvus ile aranıyor (mesafe < {distance_threshold:.4f})...")
        ...
        try:
            similar_face_ids_from_milvus = g.db_tools.find_similar_face_ids_in_milvus(
                ...
                limit=100  # Hedef kişinin tüm benzerlerini bulmak için yüksek limit (kullanıcı talebi)
            )
            if similar_face_ids_from_milvus:
                target_group_ids.update(similar_face_ids_from_milvus)
            current_app.logger.info(f"Milvus'tan hedef yüz grubu bulundu: {len(target_group_ids)} yüz (ID: {target_face_id} dahil). Bu kişinin tüm görüntüleri analiz edilecek.")

        except AttributeError as attr_err:
            current_app.logger.error(f"Milvus arama fonksiyonu g.db_tools'da bulunamadı: {attr_err}")
            flash('Milvus arama altyapısında bir sorun var (find_similar_face_ids_in_milvus). Lütfen yönetici ile iletişime geçin.', 'error')
            if conn: g.db_tools.releaseConnection(conn, cur)
            return redirect(url_for('web.dashboard'))
        except Exception as milvus_err:
            current_app.logger.error(f"Milvus arama sırasında genel hata: {milvus_err}\\{traceback.format_exc()}")
            flash(f'Milvus ile benzer yüzler aranırken bir hata oluştu: {str(milvus_err)}', 'error')
            if conn: g.db_tools.releaseConnection(conn, cur)
            return redirect(url_for('web.dashboard'))
        ...
        current_app.logger.info(f"Hedef kişinin bulunduğu TÜM görselleri bulma aşaması başlıyor...")
        target_images_query = """ 
        ...
        current_app.logger.info(f"Hedef grup yüzlerini içeren {len(target_images)} potansiyel görsel kaydı bulundu. Bu görüntülerde yer alan tüm kişiler için birlikte görülme analizi yapılacak.")
        ...
        for img in target_images:
            ...
            if img_hash not in image_hash_map:
                 image_hash_map[img_hash] = {'image_id': img.get('ImageID'), 'faces': set()}
            if img.get('FaceID'):
        ...
        current_app.logger.info(f"Tekilleştirilmiş {len(image_hash_map)} görseldeki toplam yüz sayısı: {len(all_related_face_ids)}")

        related_face_ids_to_group = all_related_face_ids - target_group_ids
        current_app.logger.info(f"Hedef grup dışındaki gruplanacak ilişkili yüz sayısı: {len(related_face_ids_to_group)}")
        ...
        if related_face_ids_to_group:
            current_app.logger.info(f"Gruplama için {len(related_face_ids_to_group)} ilişkili yüzün Milvus verileri çekiliyor...")
            ...
            current_app.logger.info(f"Toplu Milvus veri çekme tamamlandı. Süre: {batch_end_time - batch_start_time}. "
            ...
            for face_id, milvus_attrs in batch_milvus_data.items():
                if 'face_embedding_data' in milvus_attrs:
                    ...
                    if isinstance(emb_list, str): emb_list = ast.literal_eval(emb_list)
                    ...
                    if emb_vector.size == 512:
                        ...
                        current_app.logger.warning(f"Gruplama için yüz {face_id} Milvus embedding boyutu geçersiz ({emb_vector.size}). Atlanıyor.")
                else:
                    current_app.logger.warning(f"Gruplama için yüz {face_id} Milvus embedding verisi bulunamadı. Atlanıyor.")
        current_app.logger.info(f"Gruplama için {len(related_faces_milvus_data_for_grouping)} adet ilişkili yüzün Milvus embedding verisi başarıyla çekildi.")
        ...
        current_app.logger.info(f"İlişkili yüz gruplama işlemi başlıyor. Toplam {len(related_faces_milvus_data_for_grouping)} yüz için gruplama yapılacak...")
        for i, face1_data in enumerate(related_faces_milvus_data_for_grouping):
            ...
            for face2_data in related_faces_milvus_data_for_grouping[i+1:]:
                ...
                try:
                    ...
                    current_app.logger.error(f"Grup benzerlik hesaplama hatası ({face1_id}-{face2_id}): {e}")
            next_group_id += 1

        end_grouping_time = datetime.datetime.now()
        current_app.logger.info(f"İlişkili yüz gruplama tamamlandı. {next_group_id} grup oluşturuldu. Süre: {end_grouping_time - start_grouping_time}. Toplam {processed_count_grouping} karşılaştırma yapıldı.")

        # 8. Her grup için hedef kişiyle görülme sayısını hesapla (Hash bazlı)
        group_occurrences = defaultdict(int)
        current_app.logger.info("Grup-hedef birliktelik sayısı hesaplanıyor: Hangi kişi grubu, hedef kişi ile kaç kez aynı fotoğrafta görülmüş...")
        for img_hash, data in image_hash_map.items():
            current_faces = data['faces']
            if any(face_id_check in target_group_ids for face_id_check in current_faces): 
        ...
        current_app.logger.info(f"{len(group_occurrences)} farklı ilişkili kişi/grup, hedef kişi ile en az bir kez birlikte görüldü. Bu sonuçlar analiz için kullanılacak.")

        # 9. Sonuçları hazırla
        final_related_faces = []
        current_app.logger.info("Sonuçlar için detaylar (temsilci yüz, görsel vb.) çekiliyor...")
        ...
        for group_id, occurrence_count in group_occurrences.items():
            if group_id not in face_groups or not face_groups[group_id]:
                 current_app.logger.warning(f"Grup {group_id} geçersiz veya boş! Atlanıyor.")
                 continue
        ...
        current_app.logger.info(f"Temsilci yüzler için toplu Milvus verisi çekildi. Süre: {rep_end_time - rep_start_time}. "
        ...
        for group_id, occurrence_count in group_occurrences.items():
            ...
            if pg_face_details: 
                ...
                if related_image_id:
                    success_rel_img, rel_img_binary = g.db_tools.getImageBinaryByID(related_image_id)
                    if success_rel_img and rel_img_binary:
                        try:
                            decompressed_binary = decompress_image(rel_img_binary)
                            if decompressed_binary:
                                related_image_data = base64.b64encode(decompressed_binary).decode('utf-8')
                                related_mime_type = 'image/png' 
                            else:
                                current_app.logger.warning(f"İlişkili yüz ({representative_face_id}) için dekompresyon boş veri döndürdü.")
                        except Exception as rel_encode_err:
                            current_app.logger.error(f"İlişkili yüz ({representative_face_id}) resim dekompres/encode hatası: {rel_encode_err}")
                
                facebox_from_milvus = None
                if rep_milvus_attrs and 'face_box' in rep_milvus_attrs:
                    fb_data = rep_milvus_attrs['face_box']
                    if isinstance(fb_data, str): fb_data = ast.literal_eval(fb_data)
                    if fb_data and len(fb_data) == 4 : 
                         facebox_from_milvus = np.array(fb_data, dtype=np.float32).tolist()
                ...
                current_app.logger.warning(f"Temsilci yüz {representative_face_id} için PG detayları bulunamadı. Sonuçlara eklenemedi.")

        final_related_faces.sort(key=lambda x: x['co_occurrence'], reverse=True)
        current_app.logger.info(f"Sonuçlar sıralandı. {len(final_related_faces)} ilişkili yüz grubu/temsilcisi bulundu.")

        target_face_box_display = None
        if target_milvus_data and 'face_box' in target_milvus_data:
            fb_list = target_milvus_data['face_box']
            if isinstance(fb_list, str): fb_list = ast.literal_eval(fb_list)
        ...
        current_app.logger.info("Sonuçlar PDF raporu için session'a kaydedildi (görsel verisi hariç).")
        ...
        current_app.logger.error(f"Kapsamlı kişi analizi hatası: {str(e)}\\{traceback.format_exc()}")
        flash(f'Analiz sırasında bir hata oluştu: {str(e)}', 'danger')
        if conn: 
            try:
                g.db_tools.releaseConnection(conn, cur)
            except Exception as cleanup_error:
                 current_app.logger.error(f"Kapsamlı analiz hata bloğunda bağlantı kapatma hatası: {cleanup_error}")
        return redirect(url_for('web.dashboard')) 
    finally:
        if conn and cur and not conn.closed : 
            try:
                ...
                current_app.logger.error(f"Kapsamlı analiz finally bloğunda bağlantı kapatma hatası: {final_cleanup_error}")
...
def download_comprehensive_analysis_report():
    ...
    search_type = f"Kapsamlı Kişi Analizi (Hedef ID: {target_face_id}, Eşik: {threshold})"
    ...
    try:
        ...
        filtered_related_faces = [face for face in related_faces_data if face.get('co_occurrence', 0) > 1]
        ...
        for face_info in filtered_related_faces:
            ...
            image_data_b64 = None
            image_url = None 
            db_pg_data = None # db_extra_data yerine db_pg_data kullanalım tutarlılık için
            ...
            try:
                ...
                if not milvus_attributes:
                    current_app.logger.warning(f"[PDF_COMP] Milvus'tan {res_face_id} için öznitelik alınamadı.")
            except Exception as e:
                current_app.logger.error(f"[PDF_COMP] Milvus'tan {res_face_id} için öznitelik alınırken hata: {e}")
            ...
            risk_level_final = db_pg_data.get('risk_level_db') if db_pg_data else face_info.get('risk_level') # session daki face_info dan risk_level alabiliriz
            ...
            if binary_image_data:
                try:
                    ...
                    current_app.logger.error(f"[PDF_COMP] Error encoding binary image for face {res_face_id}: {b64_error}")
                    image_data_b64 = None
                    if db_pg_data: 
                        image_url = build_image_url(
        ...
        if conn: g.db_tools.releaseConnection(conn, cursor)
        return redirect(url_for('web.comprehensive_person_analysis', face_id=target_face_id))
    finally:
        if conn: g.db_tools.releaseConnection(conn, cursor)
...
def face_relationship_details(face_id, target_face_id):
    ...
    try:
        ...
        for data in embedding_data:
            face_embeddings[data['ID']] = np.array(ast.literal_eval(data['FaceEmbeddingData']), dtype=np.float32)
        ...
        for other_face in all_other_faces:
            ...
            other_embedding = np.array(ast.literal_eval(other_face['FaceEmbeddingData']), dtype=np.float32)
            if other_embedding.size == 512:
                # Hedef yüze benzerlik
                similarity_to_target = np.dot(target_embedding, other_embedding) / (np.linalg.norm(target_embedding) * np.linalg.norm(other_embedding))
                if similarity_to_target >= similarity_threshold:
                    target_group_ids.add(other_id)
                
                # İlişkili yüze benzerlik
                similarity_to_face = np.dot(face_embedding, other_embedding) / (np.linalg.norm(face_embedding) * np.linalg.norm(other_embedding))
        ...
        current_app.logger.info(f"[REL_DETAILS] Found {len(images)} potential images containing faces from both groups.")
            ...
            if analysis_data and analysis_data.get('target_face_id') == target_face_id:
                ...
                for face_info in related_faces_data:
                    if face_info.get('id') == face_id:
                        ...
                        current_app.logger.info(f"[REL_DETAILS] Found expected co_occurrence from session: {expected_co_occurrence}")
                        break
            else:
                current_app.logger.warning(f"[REL_DETAILS] Session data mismatch or not found for target_face_id {target_face_id}.")
        except Exception as session_err:
             current_app.logger.error(f"[REL_DETAILS] Error reading session data: {session_err}")
        ...
        def create_placeholder_image(width=300, height=200, text="Görsel Yok"):
            placeholder = np.full((height, width, 3), (220, 220, 220), dtype=np.uint8) # Açık gri
            ...
            cv2.putText(placeholder, text, (text_x, text_y), font, font_scale, text_color, text_thickness, cv2.LINE_AA)
        ...
        for img in images: # Loop through rows from DB query
            ...
            try: # Outer try for the entire image processing
                ...
                if image_url:
                    ...
                    current_app.logger.debug(f"[REL_DETAILS_LOOP] Attempting download from URL: {image_url}")
                    try: # Inner try for download operation
                        download_success, downloaded_data, _ = downloadImage_defaultSafe(image_url)
                        if download_success and downloaded_data is not None:
                            if isinstance(downloaded_data, np.ndarray) and downloaded_data.size > 0:
                                cv_image = downloaded_data
                                current_app.logger.debug(f"[REL_DETAILS_LOOP] Got cv2 image directly for URL: {image_url}")
                            elif isinstance(downloaded_data, bytes):
                                image_array = np.frombuffer(downloaded_data, dtype=np.uint8)
                                cv_image = cv2.imdecode(image_array, cv2.IMREAD_COLOR)
                                if cv_image is None or cv_image.size == 0:
                                     current_app.logger.warning(f"[REL_DETAILS_LOOP] Failed to decode downloaded bytes from URL: {image_url}")
                                     placeholder_text = "URL Decode Hatası"
                                else:
                                    current_app.logger.debug(f"[REL_DETAILS_LOOP] Successfully decoded downloaded bytes from URL: {image_url}")
                            else:
                                current_app.logger.warning(f"[REL_DETAILS_LOOP] Unexpected data type from download ({type(downloaded_data)}) for URL: {image_url}")
                                cv_image = None
                                placeholder_text = "İndirme Veri Tipi Hatası"
                        else:
                            current_app.logger.warning(f"[REL_DETAILS_LOOP] Download failed or empty data received for URL: {image_url}")
                            placeholder_text = "URL İndirme Hatası"
                            cv_image = None
                    except Exception as download_err:
                        current_app.logger.error(f"[REL_DETAILS_LOOP] Error during URL download/processing {image_url}: {download_err}")
                        placeholder_text = "URL İşleme İstisnası"
                        cv_image = None
                else:
                    current_app.logger.debug(f"[REL_DETAILS_LOOP] Image URL missing for {image_source_info}. Skipping URL.")
                ...
                if cv_image is None and image_id:
                    current_app.logger.debug(f"[REL_DETAILS_LOOP] URL failed/missing. Attempting DB fetch for ImageID: {image_id}")
                    try: # Inner try for DB fetch
                        ...
                        if success_db and img_binary:
                            ...
                            if cv_image is not None and cv_image.size > 0:
                                current_app.logger.debug(f"[REL_DETAILS_LOOP] Successfully decoded image from DB (ImageID: {image_id}).")
                                placeholder_text = "" # Reset placeholder text if successful
                            else:
                                current_app.logger.warning(f"[REL_DETAILS_LOOP] Failed to decode image from DB (ImageID: {image_id})")
                                cv_image = None
                                placeholder_text = "DB Decode Hatası"
                        else:
                            current_app.logger.warning(f"[REL_DETAILS_LOOP] Failed to fetch image from DB (ImageID: {image_id})")
                            placeholder_text = "DB Getirme Hatası"
                            cv_image = None # Ensure cv_image is None if fetch fails
                    except Exception as db_fetch_err:
                         current_app.logger.error(f"[REL_DETAILS_LOOP] Error fetching/decoding DB ImageID {image_id}: {db_fetch_err}")
                         cv_image = None
                         placeholder_text = "DB İstisnası"
                elif cv_image is None:
                    current_app.logger.warning(f"[REL_DETAILS_LOOP] No image data source found for {image_source_info}")
                ...
                if cv_image is not None and cv_image.size > 0:
                    ...
                    if faces_in_image_ids:
                        ...
                        face_box_map = {fb['ID']: fb['FaceBox'] for fb in face_boxes_results}
                            ...
                            if facebox_bytes:
                                ...
                                if is_target or is_related:
                                    try: # Inner try for facebox processing
                                        facebox = np.array(ast.literal_eval(facebox_bytes), dtype=np.float32)
                                        if len(facebox) == 4:
                                         ...
                                         current_app.logger.error(f"[REL_DETAILS_LOOP] Error processing facebox for face {face_id_in_image} in image {image_id}: {fb_err}")
                    ...
                    if success_encode:
                        ...
                        current_app.logger.error(f"[REL_DETAILS_LOOP] Failed to encode processed image: {image_source_info}")
                        is_placeholder = True
            ...
            if is_placeholder or not processed_image_b64:
                ...
                images_metadata_list.append({
                    ...
                    'domain': domain_name,
                    'title': image_title,
                    'detection_date': detection_date.isoformat() if detection_date else None, # ISO format for session
                 ...
                 if faces_in_image_ids:
                     for f_id in faces_in_image_ids:
                         f_type = "Hedef" if f_id in target_group_ids else ("İlişkili" if f_id in face_group_ids else "Diğer")
        ...
        stats = {
            'total_images': expected_co_occurrence if expected_co_occurrence != -1 else len(images),
        ...
        if not target_face_dict: target_face_dict = {'ID': target_face_id}
        if not related_face_dict: related_face_dict = {'ID': face_id}
        ...
        try:
            ...
            current_app.logger.info(f"[REL_DETAILS] Successfully saved data to session key: {session_key}")
        except Exception as session_save_err:
            # Oturuma kaydetme hatası kritik değil, devam et ama logla
            current_app.logger.error(f"[REL_DETAILS] Failed to save relationship details to session: {session_save_err}")
            flash("Rapor verileri oturuma kaydedilemedi, PDF indirme çalışmayabilir.", "warning")
        # ---> Session Kaydı Bitti <---
        ...
        current_app.logger.error(f"İlişki detayları hatası: {str(e)}\n{traceback.format_exc()}")
        flash(f'İlişki detayları alınırken bir hata oluştu: {str(e)}', 'danger')
        # Hata durumunda bağlantıyı kapatmayı dene
        try:
            if conn: g.db_tools.releaseConnection(conn, cur)
        except Exception as cleanup_err:
            current_app.logger.error(f"[REL_DETAILS] Error during cleanup in except block: {cleanup_err}")
        # Kapsamlı analize geri yönlendir
        tfi = 0
        try:
             tfi = int(target_face_id)
        except (ValueError, TypeError):
             current_app.logger.warning(f"[REL_DETAILS] Could not convert target_face_id '{target_face_id}' to int for redirect.")
...
@web_bp.route('/download/relationship_details_report/<int:target_face_id>/<int:face_id>', methods=['GET'])
@login_required
def download_relationship_details_report(target_face_id, face_id):
    ...
    if not relationship_data:
        flash('İndirilecek rapor verisi bulunamadı veya oturum süresi doldu.', 'warning')
        return redirect(url_for('web.face_relationship_details', target_face_id=target_face_id, face_id=face_id))
    ...
    pdf_data = []
    conn = None # Veritabanı bağlantısı gerekebilir (yüz kutucuklarını tekrar almak için)
    cursor = None
    ...
    try:
        ...
        # 1. Embedding'leri al
        cursor.execute('SELECT "ID", "FaceEmbeddingData" FROM "EyeOfWebFaceID" WHERE "ID" IN (%s, %s)', (target_face_id, face_id))
        embedding_data = cursor.fetchall()
        if len(embedding_data) < 2: raise ValueError("Hedef veya ilişkili yüz embedding bulunamadı")
        face_embeddings = {data['ID']: np.array(ast.literal_eval(data['FaceEmbeddingData']), dtype=np.float32) for data in embedding_data}
        
        # 2. Benzer yüzleri bul
        target_group_ids = {target_face_id}
        face_group_ids = {face_id}
        cursor.execute('SELECT "ID", "FaceEmbeddingData" FROM "EyeOfWebFaceID" WHERE "ID" != %s AND "ID" != %s AND "FaceEmbeddingData" IS NOT NULL', (target_face_id, face_id))
        ...
        for other_face in all_other_faces:
            ...
            if other_embedding.size == 512:
                similarity_to_target = np.dot(target_embedding, other_embedding) / (np.linalg.norm(target_embedding) * np.linalg.norm(other_embedding))
                if similarity_to_target >= threshold: target_group_ids.add(other_id)
                similarity_to_face = np.dot(face_embedding, other_embedding) / (np.linalg.norm(face_embedding) * np.linalg.norm(other_embedding))
                if similarity_to_face >= threshold: face_group_ids.add(other_id)
        # --- Grup Oluşturma Bitti ---
        
        total_meta_items = len(images_metadata_list)
        processed_successfully = 0
        used_placeholder_count = 0
        current_app.logger.info(f"[PDF_REL_REPORT] Starting PDF generation for {total_meta_items} image metadata items.")

        for idx, img_meta in enumerate(images_metadata_list):
            log_prefix = f"[PDF_REL_LOOP {idx+1}/{total_meta_items}]" # Log prefix for this item
            current_app.logger.info(f"{log_prefix} Processing image meta: ID={img_meta.get('id', 'N/A')}, URL={img_meta.get('url', 'N/A')}")
            ...
            detection_date = img_meta.get('detection_date') # Keep as string/datetime from session for now
            ...
            if session_error_msg:
                 current_app.logger.warning(f"{log_prefix} Skipping fetch, error from session: {session_error_msg}")
                 is_placeholder = True
            else:
                ...
                try:
                    ...
                    if original_url:
                        ...
                        current_app.logger.debug(f"{log_prefix} Attempting download from URL...")
                        try:
                            dl_success, dl_data, _ = downloadImage_defaultSafe(original_url)
                            if dl_success and dl_data is not None:
                                if isinstance(dl_data, np.ndarray) and dl_data.size > 0:
                                    cv_image = dl_data
                                    current_app.logger.debug(f"{log_prefix} Success: Got cv2 image directly from URL.")
                                    ...
                                    if cv_image is None or cv_image.size == 0:
                                        current_app.logger.warning(f"{log_prefix} Failure: Could not decode bytes from URL.")
                                        placeholder_text = "URL Decode H."
                                    else:
                                         current_app.logger.debug(f"{log_prefix} Success: Decoded bytes from URL.")
                                else:
                                    current_app.logger.warning(f"{log_prefix} Failure: Unexpected data type from URL download: {type(dl_data)}")
                                    cv_image = None; placeholder_text = "URL Veri Tipi H."
                            else:
                                current_app.logger.warning(f"{log_prefix} Failure: Download failed or empty data from URL.")
                                cv_image = None; placeholder_text = "URL İndirme H."
                        except Exception as download_err:
                            current_app.logger.error(f"{log_prefix} Exception during URL download: {download_err}")
                            cv_image = None; placeholder_text = "URL İstisna H."
                    else:
                        current_app.logger.debug(f"{log_prefix} URL missing in metadata.")
                        placeholder_text = "URL Yok (Meta)"; cv_image = None

                    # 2. Try DB if URL failed and ImageID exists
                    if cv_image is None and image_id:
                        current_app.logger.debug(f"{log_prefix} Attempting DB fetch for ImageID: {image_id}")
                        try:
                            success_db, img_binary = g.db_tools.getImageBinaryByID(image_id)
                            if success_db and img_binary:
                                image_array = np.frombuffer(img_binary, dtype=np.uint8)
                                cv_image = cv2.imdecode(image_array, cv2.IMREAD_COLOR)
                                if cv_image is not None and cv_image.size > 0:
                                    current_app.logger.debug(f"{log_prefix} Success: Decoded image from DB.")
                                    placeholder_text = "" # Reset if successful
                                else:
                                    current_app.logger.warning(f"{log_prefix} Failure: Failed to decode image from DB.")
                                    cv_image = None; placeholder_text = "DB Decode H."
                            else:
                                current_app.logger.warning(f"{log_prefix} Failure: Failed to fetch image from DB.")
                                cv_image = None; placeholder_text = "DB Getirme H."
                        except Exception as db_fetch_err:
                            current_app.logger.error(f"{log_prefix} Exception during DB fetch: {db_fetch_err}")
                            cv_image = None; placeholder_text = "DB İstisna H."
                    elif cv_image is None:
                         current_app.logger.warning(f"{log_prefix} No valid image source could be used (URL failed/missing and no ImageID).")

                    # --- Process image if obtained ---
                    if cv_image is not None and cv_image.size > 0:
                        current_app.logger.debug(f"{log_prefix} Image obtained, processing faceboxes...")
                        try:
                            img_height, img_width = cv_image.shape[:2]
                            face_ids_in_image = [f['id'] for f in faces_metadata]
                            if face_ids_in_image:
                                 query_placeholders = ",".join(['%s'] * len(face_ids_in_image))
                                 # Use tuple for query parameters
                                 cursor.execute(f'SELECT "ID", "FaceBox" FROM "EyeOfWebFaceID" WHERE "ID" IN ({query_placeholders})', tuple(face_ids_in_image))
                                 ...
                                 for face_meta in faces_metadata:
                                     ...
                                     if facebox_bytes:
                                         try:
                                             ...
                                             current_app.logger.error(f"{log_prefix} Error processing facebox {face_id_in_image}: {fb_proc_err}")
                                 current_app.logger.debug(f"{log_prefix} Drew {drawn_boxes} boxes.")

                            # Encode to base64
                            success_encode, buffer = cv2.imencode('.jpg', cv_image)
                            if success_encode:
                                processed_image_b64 = base64.b64encode(buffer).decode('utf-8')
                                current_app.logger.debug(f"{log_prefix} Image successfully processed and encoded.")
                                processed_successfully += 1
                            else:
                                current_app.logger.error(f"{log_prefix} Failed to encode processed image.")
                                is_placeholder = True; placeholder_text = "Encode H."
                        except Exception as img_proc_err:
                             current_app.logger.error(f"{log_prefix} Exception during image processing (boxes/encode): {img_proc_err}")
                             is_placeholder = True; placeholder_text = "İşleme Hatası"
                    else:
                        # Could not obtain image
                        current_app.logger.warning(f"{log_prefix} Using placeholder as no valid image was obtained.")
                        is_placeholder = True

                except Exception as outer_proc_error:
                    current_app.logger.error(f"{log_prefix} Outer exception during image processing: {outer_proc_error}\n{traceback.format_exc()}")
                    is_placeholder = True; placeholder_text = "Genel Hata"

            # --- Create PDF Item ---
            if is_placeholder or not processed_image_b64:
                 current_app.logger.warning(f"{log_prefix} Final decision: Using placeholder ({placeholder_text}).")
                 processed_image_b64 = create_placeholder_image(text=placeholder_text)
                 used_placeholder_count += 1
                 if not processed_image_b64:
                      current_app.logger.error(f"{log_prefix} CRITICAL: Failed to create even placeholder image!")
            ...
            try:
                 ...
                 if isinstance(detection_date, str):
                    try: 
                         date_obj = datetime.datetime.fromisoformat(detection_date.replace('Z', '+00:00')) 
                         date_str = date_obj.strftime('%d.%m.%Y %H:%M')
                    except ValueError: # Handle non-ISO format if necessary
                         try:
                              date_obj = datetime.datetime.strptime(detection_date, '%Y-%m-%d %H:%M:%S')
                              date_str = date_obj.strftime('%d.%m.%Y %H:%M')
                         except ValueError:
                              current_app.logger.warning(f"{log_prefix} Unknown date format: {detection_date}")
                 elif isinstance(detection_date, datetime.datetime):
                    date_str = detection_date.strftime('%d.%m.%Y %H:%M')
                 if date_str : title += f" ({date_str})"
            except Exception as date_fmt_err:
                 current_app.logger.warning(f"{log_prefix} Could not format detection date '{detection_date}': {date_fmt_err}")

            # Format face info string
            face_info_str_parts = []
            for face in faces_metadata:
                 f_id = face.get('id'); f_type = "H" if f_id in target_group_ids else ("İ" if f_id in face_group_ids else "D")
                 face_info_str_parts.append(f"#{f_id}({f_type})")
            face_info_str = ", ".join(face_info_str_parts) if face_info_str_parts else "Yok"
            ...
            pdf_item = {
                ...
                'score': None, 'gender': None, 'age': None, 'similarity': None,
        ...
        # --- Loop End ---
        current_app.logger.info(f"[PDF_REL_REPORT] Loop finished. Processed: {processed_successfully}, Placeholders: {used_placeholder_count}/{total_meta_items}")

    except (Exception, psycopg2.DatabaseError) as error:
        current_app.logger.error(f"Error preparing relationship PDF data: {error}\n{traceback.format_exc()}")
...
def download_detection_report():
    ...
    # PDF dosyasını kullanıcıya gönder
    report_filename = f"EyeOfWeb_YuzTespit_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    return send_file(
        io.BytesIO(pdf_bytes),
        mimetype='application/pdf',
        as_attachment=True,
...
def download_comparison_report():
    ...
    username = report_data.get('username', 'Bilinmeyen Kullanıcı')
    pdf_data_list = report_data.get('pdf_data', [])
    comparison_details = report_data.get('comparison_info', None) # Get comparison details
    ...
    pdf_bytes = generate_pdf_report(search_type, username, pdf_data_list, comparison_info=comparison_details)
